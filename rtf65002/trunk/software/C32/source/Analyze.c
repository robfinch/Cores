...
void AllocateRegisterVars()
{
    ...
    while( csp != NULL ) {
        if( OptimizationDesireability(csp) < 3 )	// was < 3
            csp->reg = -1;
        else if( csp->duses > csp->uses / 4 && reg < 5 )
//        else if( reg < 5 )
            csp->reg = reg++;
        else
            csp->reg = -1;
	...
	if( mask != 0 ) {
		cnt = 0;
		GenerateDiadic(op_subsp,0,makereg(REG_SP),make_immed(popcnt(rmask)*1));
		for (nn = 0; nn < 16; nn++) {
			if (rmask & (0x8000 >> nn)) {
				GenerateDiadic(op_st,0,makereg(nn),make_indexed(cnt,REG_SP));
				cnt+=1;
			}
		}
		//else {
  ...
  //          GenerateDiadic(op_sm,0,make_indirect(30),make_mask(mask),NULL);
    ...
    while( csp != NULL )
	{
            ...
            {               /* see if preload needed */
                    ...
                    {
                            initstack();
                            ap = GenerateExpression(exptr,F_ALL,1);
                            ap2 = makereg(csp->reg);
