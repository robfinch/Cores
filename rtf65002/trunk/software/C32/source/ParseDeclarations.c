...
char            *declid = NULL;
TABLE           tagtable = {0,0};
TYP             stdconst = { bt_long, bt_long, 1, FALSE, FALSE, FALSE, 0, 0, 1, {0, 0}, 0, "stdconst"};
char *names[20];
int nparms = 0;
int funcdecl = FALSE;
int catchdecl = FALSE;
...
void ParseSpecifier(TABLE *table)
{
	...
	for (;;) {
		switch (lastst) {
			...
			case kw_naked:
				isNocall = TRUE;
				head = tail = maketype(bt_oscall,1);
				NextToken();
				goto lxit;
			...
			case kw_interrupt:
				isInterrupt = TRUE;
				head = tail = maketype(bt_interrupt,1);
				NextToken();
				goto lxit;

			case kw_pascal:
				isPascal = TRUE;
				head = tail = maketype(bt_pascal,1);
				NextToken();
				break;
			...
			case kw_long:	// long, long int
				...
				if (lastst==kw_float)
					head = tail = maketype(bt_double,1);
				else
					head = tail = maketype(bt_long,1);
				NextToken();
				if (lastst==kw_oscall) {
					isOscall = TRUE;
					NextToken();
...
void ParseAutoDeclarations()
{
    ...
    for(;;) {
        ...
        case kw_thread:
                NextToken();
				lc_thread += declare(&lsyms,sc_thread,lc_thread,bt_struct);
				break;
        case kw_static:
                NextToken();
