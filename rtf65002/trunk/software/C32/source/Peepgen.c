...
{
	if (ip==NULL) {
		printf("NULL pointer in peep_cmp.\r\n");
		return;
	}
	if (ip->back) {
		// Comparing to register zero ?
		if (ip->oper1->mode == am_reg && ip->oper1->preg==0) {
			if (ip->back->opcode == op_ld && ip->back->oper1->mode==am_reg &&
				ip->oper2->mode==am_reg && ip->back->oper1->preg == ip->oper2->preg) {
				ip->back->fwd = ip->fwd;
				ip->fwd->back = ip->back;
			}
		}
...
// If a store is followed by a load of the same address, then remove the load
// operation unless it's a volatile address.
//
void peep_ld(struct ocode *ip)
{
	return;
	if (ip==NULL) {
		printf("NULL pointer in peep_ld.\r\n");
		return;
	}
	if (ip->back==NULL)
		return;
	if (ip->back->opcode != op_st)
		return;
	if (ip->back->oper1->preg != ip->oper1->preg)
		return;
	// Is it the same addressing mode, offset and register ?
	if (ip->back->oper2->mode != ip->oper2->mode)
		return;
	if (ip->back->oper2->preg != ip->oper2->preg)
		return;
	if (ip->back->oper2->offset->nodetype != ip->oper2->offset->nodetype)
		return;
	if (ip->back->oper2->offset->nodetype != en_icon)
		return;
	if (ip->back->oper2->offset->i != ip->oper2->offset->i)
		return;
	if (ip->back->oper2->offset->isVolatile || ip->oper2->offset->isVolatile)
		return;
	ip->back->fwd = ip->fwd;
	if (ip->fwd)
		ip->fwd->back = ip->back;
...
void peep_bra(struct ocode *ip)
{
	if (ip==NULL) {
		printf("NULL pointer in peep_bra.\r\n");
		return;
	}
	PeepoptUctran(ip);
	// check for a branch to the next opcode
	if (ip->fwd==NULL)
		return;
	if (ip->fwd->opcode != op_label)
		return;
	if (ip->oper1->offset->i == ip->fwd->oper1) {
		if (ip->back)
			ip->back->fwd = ip->fwd;
		ip->fwd->back = ip->back;
	}
