...
{    
    ...
    GenerateDiadic(op_jsr,0,make_strlab(lib_name),NULL);
}
...
{    
	...
	int nn;
        ...
        defcase = 0;
        initstack();
        ap = GenerateExpression(stmt->exp,F_ALL,1);
        if( ap->preg != 0 )
        ...
        while( stmt != NULL )
        {
            ...
            else
            {
				bf = stmt->label;
				for (nn = bf[0]; nn >= 1; nn--) {
					GenerateMonadic(op_cmp,0,make_immed(bf[nn]));
					GenerateMonadic(op_beq,0,make_label(curlab));
				}
...
void GenerateTry(Statement *stmt)
{
	...
	while (stmt) {
		...
		if (stmt->s2==99999)
			...
			GenerateDiadic(op_cmp,0,makereg(2),make_immed((int)stmt->s2));
			GenerateMonadic(op_bne,0,make_label(nextlabel));
		}
		// move the throw expression result in 'r1' into the catch variable.
		sym = (SYM *)stmt->label;
		if (sym) {
			GenerateDiadic(op_st,0,makereg(1),make_indexed(sym->value.i,27));
		}
		GenerateStatement(stmt->s1);
		stmt=stmt->next;
	}
    GenerateLabel(lab1);
	GenerateDiadic(op_lea,0,makereg(REG_XL),make_label(oldthrow));
}
...
void GenerateThrow(Statement *stmt)
{
    ...
    if( stmt != NULL && stmt->exp != NULL )
	{
		...
		if (ap->mode==am_immed)
			...
			GenerateTriadic(op_ld,0,makereg(1),ap,makereg(0));
...
void GenerateSpinlock(Statement *stmt)
{
    ...
    if( stmt != NULL && stmt->exp != NULL )
	{
		...
		if (stmt->initExpr)
			GenerateDiadic(op_ld, 0, ap1,make_immed(stmt->initExpr));
		GenerateLabel(lab1);
		if (stmt->initExpr) {
			GenerateTriadic(op_sub, 0, ap1, ap1, make_immed(1));
			GenerateTriadic(op_beq, 0, ap1, makereg(0), make_label(lab2));
		}
		GenerateDiadic(op_ld, 0, ap2, make_indexed(stmt->incrExpr,ap->preg));
		GenerateTriadic(op_beq, 0, ap2, makereg(0), make_label(lab1));
		ReleaseTempRegister(ap2);
		GenerateStatement(stmt->s1);
		// unlock
		GenerateDiadic(op_st, 0, makereg(0), make_indexed(stmt->incrExpr,ap->preg));
		if (stmt->initExpr) {
    ...
    if( stmt != NULL && stmt->exp != NULL )
	{
		...
		if( ap->preg != 1 ) {
			if (ap->mode == am_immed)
				...
				GenerateDiadic(op_ld, 0, makereg(1),ap);
			GenerateDiadic(op_st, 0, makereg(0),make_indexed(stmt->incrExpr,1));
		}
	}
...
void GenerateStatement(struct snode *stmt)
{
    ...
    //                    break;
                ...
                case st_expr:
                        initstack();
                        ap1 = GenerateExpression(stmt->exp,F_ALL | F_NOVALUE,
