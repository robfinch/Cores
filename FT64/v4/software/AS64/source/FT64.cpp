...
static void process_riop(int64_t opcode6)
{
	...
	if (!IsNBit(val,20)) {
		...
		emit_insn(
			(opcode6 << 34LL) |
			(sz << 24) |		// set size to word size op
			(Rt << 18) |
			(52 << 12) |
			(Ra << 6) |
			0x02,!expand_flag,5);
	...
	emit_insn(((val & 0xFFFFF) << 20)|((sz & 3) << 18) | (Rt << 12)|(Ra << 6)|opcode6,!expand_flag,5);
}
...
static void process_shifti(int64_t op4)
{
     ...
     NextToken();
     val = expr();
	emit_insn((func6 << 34LL) | (op4 << 30LL) | (sz << 24) | ((val & 0x3f) << 18) | (Rt << 12) | (Ra << 6) | 0x02,!expand_flag,5);
}
