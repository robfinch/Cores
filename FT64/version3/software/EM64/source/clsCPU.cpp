...
void clsCPU::Reset()
	...
	void clsCPU::Step()
	{
		...
		if (nmi && !StatusHWI)
			...
			// Read the pc in two halves as pc is bit pair aligned.
			ix1 = system1->Read(pc >> 2);
			ix2 = system1->Read((pc + 4) >> 2);
			switch (pc & 3) {
			case 0:	sir = ir = ix1 + ((ix2 & 15) << 32);
			case 1:	sir = ir = (ix1 >> 2) + ((ix2 & 63) << 30);
			case 2:	sir = ir = (ix1 >> 4) + ((ix2 & 255) << 28);
			case 3:	sir = ir = (ix1 >> 6) + ((ix2 & 2047) << 26);
			}
			opc = pc;
			if (ir & 0x80)
				pc <= pc + 9;
