...
#pragma endregion
	private: System::Void frmAsmDisplay_Paint(System::Object^  sender, System::Windows::Forms::PaintEventArgs^  e) {
				...
				static unsigned __int64 old_ad = -1;
				char buf[200];
				char *buf2;
				std::string str;
				unsigned int row;
				int xx, yy;
				unsigned __int64 ad1, ad2;
				unsigned __int64 dat;
				unsigned int datx, daty;
				...
				//if (ad2 != old_ad)
				{
					...
					for (row = 0; row < 32; row++) {
						yy = row * 12 + 10;
						sprintf(buf,"%08I64X.%01I64X", ad2 >> 2LL, (ad2 & 3LL) << 2LL);
						str = std::string(buf);
						dat = system1.ReadInsn(ad2);
			...
			//			dat = system1.memory[ad>>2];
						if (ad2==cpu1.pc) {
							...
							if (dat & 0x80LL)
								sprintf(buf, "%05I64X", dat & 0x3FFFFLL);
							else
								sprintf(buf,"%09I64X", dat);
							str = std::string(buf);
							gr->DrawString(gcnew String(str.c_str()),myfont,bkbr,xx + 74,yy);
							str = Disassem(ad2,dat,&ad1);
							gr->DrawString(gcnew String(str.c_str()),myfont,bkbr,xx + 138,yy);
						}
						else {
							...
							if (dat & 0x80LL)
								...
								sprintf(buf, "%09I64X", dat);
							str = std::string(buf);
							gr->DrawString(gcnew String(str.c_str()),myfont,fgbr,xx + 74,yy);
							str = Disassem(ad2,dat,&ad1);
							gr->DrawString(gcnew String(str.c_str()),myfont,fgbr,xx + 138,yy);
						}
						if (dat & 0x80LL)
							ad2 = ad2 + 9LL;
						else
					...
					yy = 17 * 12 + 10;
					sprintf(buf, "pc %08I64X.%01I64X", cpu1.pc >> 2LL, (cpu1.pc & 3LL) << 2LL);
					str = std::string(buf);
					gr->DrawString(gcnew String(str.c_str()),myfont,fgbr,310,yy);
