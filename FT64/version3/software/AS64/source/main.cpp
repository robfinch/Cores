...
extern void FT64v3_processMaster();
...
void displayHelp()
{
     ...
     printf("                          D=DSD6 7=DSD7 A=DSD9 F=FT64 T3=FT64v3 G=FT64x36\r\n");
...
//   qsort(pt, htblmax, sizeof(HTBLE), hcmp);
   ...
   if (gCpu=='H') {
		...
		fprintf(ofp, "The top 8192 are:\n", htblmax);
		fprintf(ofp, "Comp  Opcode  Count\n"); 
		for (nn = 0; nn < htblmax && nn < 8192; nn++) {
			fprintf(ofp, " %03X %09I64X %d\n", nn, (hTable[nn].realOpcode ? hTable[nn].realOpcode : hTable[nn].opcode) & 0xFFFFFFFFFLL, hTable[nn].count);
		}
		return;
   }
    fprintf(ofp, "%d compressable instructions\n", htblmax);
...
*/
    ...
    if (segment == codeseg || segment == rodataseg) {
		{
			binfile[binndx] &= ~(3 << code_bit_ndx);
			binfile[binndx] |= (cd & 3LL) << code_bit_ndx;
			if (code_bit_ndx==6)
				binndx++;
		}
    }
...
static void SetVal(SYM *sym, Int128 ca, int64_t bit)
{
	if (gCpu=='G')
		sym->value.low = ca.low & -4LL;
	else if (gCpu=='H' && segment==codeseg) {
		sym->value.frac = ca.frac;
		sym->value.low = ca.low;
	}
	else {
		sym->value.frac = ca.frac;
		sym->value.low = ca.low;
...
static void CheckVal(SYM *sym, Int128 ca, int64_t bit)
{
	int64_t t;

	if (gCpu=='H' && !ca.IsEqual(&ca,&sym->value)) {
        phasing_errors++;
        sym->phaserr = '*';
	}
	else if ((sym->value.low != ca.low && gCpu!='G') || (sym->value.low != (ca.low & -4LL) && gCpu=='G')) {
        phasing_errors++;
        sym->phaserr = '*';
...
//        ca = code_address;
    ...
    switch(segment) {
    case codeseg:
         ca.low = code_address/div;
         ca.low = sections[0].address/div;
		 ca.frac = sections[0].bit_index << 1;
         break;
    case rodataseg:
         ca.low = sections[1].address/div;
         break;
    case dataseg:
         ca.low = sections[2].address/div;
         break;
    case bssseg:
         ca.low = sections[3].address/div;
         break;
    case tlsseg:
         ca.low = sections[4].address/div;
         break;
    }
    NextToken();
    if (token != tk_id) {
...
void process_org()
{
    ...
    if (!rel_out) {
        if (segment==dataseg) {
            data_address = new_address.low *mul;
            sections[segment].address = new_address.low *mul;
        }
        else if (segment==bssseg || segment==tlsseg) {
            bss_address = new_address.low*mul;
            sections[segment].address = new_address.low*mul;
        }
        else {
            if (first_org && segment==codeseg) {
				code_address = new_address.low;
				code_bit_ndx = new_address.frac >> 1;
				if (gCpu=='H') {
					start_address = new_address.low;
					start_bitndx = new_address.frac >> 1;
					sections[0].address = new_address.low;
					sections[0].bit_index = new_address.frac >> 1;
				}
				else {
					start_address = new_address.low*mul;
					start_bitndx = 0;
					sections[0].address = new_address.low*mul;
				}
               first_org = 0;
            }
            else {
                 ...
                 if (!isInitializationData) {
					 if (gCpu=='H') {
					while(sections[0].address < new_address.low) {
						emitBitPair(0);
					}
					while (sections[0].bit_index < (new_address.frac >> 1))
						emitBitPair(0);
					 }
					 else {
					while(sections[0].address < new_address.low*mul) {
						emitByte(0x00);
					}
					 }
				 }
...
void process_align()
{
	...
	if (gCpu=='H') {
		if (segment == rodataseg || segment == dataseg || segment==bssseg || segment==tlsseg) {
			while (sections[segment].address % v.low)
				emitByte(0x00);
		}
		else if (segment == codeseg) {
			if (alignCache) {
				for (count = 0; count < 32* 4; count++) {
					a.frac = sections[segment].bit_index << 1;
					a.low = sections[segment].address;
					a.high = 0;
					if (a.frac == 0 && (a.low & 0x1F) == 0)
						break;
					emitBitPair(0x00);
				}
			}
...
void process_dh_htbl()
{
	...
	if (gCpu=='H') {
		emitHalf(htblmax > 8192 ? 8192 : htblmax);
		for (nn = 0; nn < htblmax && nn < 8192; nn++) {
			if (hTable[nn].realOpcode != 0)
				emitWord(hTable[nn].realOpcode);
			else
...
//    printf("<process_label>");
    ...
    switch(segment) {
    case codeseg:
         ca.low = code_address >> shft;
         ca.low = sections[0].address >> shft;
		 ca.frac = sections[0].bit_index << 1;
         break;
    case rodataseg:
         ca.low = sections[1].address >> shft;
         break;
    case dataseg:
         ca.low = sections[2].address >> shft;
         break;
    case bssseg:
         ca.low = sections[3].address >> shft;
         break;
    case tlsseg:
         ca.low = sections[4].address >> shft;
         break;
    }
//    if (segment==bssseg)
//       ca = bss_address;
...
//       prevToken();
    ...
    if (pass==4 || pass==3) {
        if (sym) {
            ...
            sym->defined = 1;
            if (isEquate) {
				sym->value.Assign(&sym->value, &val);
...
void processMaster()
{
	...
	case 'H':	FT64v3_processMaster();	break;
