...
private: void UpdateForm() {
				 ...
				 jj = 0;
				 if (lastSP != cpu1.regs[31][cpu1.rgs]) {
					 sprintf(buf, "%06X", cpu1.regs[31][cpu1.rgs]);
					 this->textBoxSP->Text = gcnew String(buf);
					 this->listBoxSP->Items->Clear();
				     for (nn = -128; nn < 128; nn+=8) {
						 kk = max(0,cpu1.regs[31][cpu1.rgs]+nn);
						 sprintf(buf, "%s %06X: %016I64X", kk == cpu1.regs[31][cpu1.rgs] ? "SP->" : "    ", (unsigned int)kk,
							 ((unsigned __int64)system1.Read(kk+4) << 32)|(unsigned __int64)system1.Read(kk));
						 this->listBoxSP->Items->Add(gcnew String(buf));
						 if (kk == cpu1.regs[31][cpu1.rgs])
							 this->listBoxSP->SetSelected(jj,true);
						 lastSP = cpu1.regs[31][cpu1.rgs];
						 jj++;
					 }
				 }
				 jj = 0;
				 if (lastBP != cpu1.regs[30][cpu1.rgs]) {
					 sprintf(buf, "%06X", cpu1.regs[30][cpu1.rgs]);
					 this->textBoxBP->Text = gcnew String(buf);
					 this->listBoxBP->Items->Clear();
					 for (nn = -128; nn < 128; nn+=8) {
						 kk = max(0,cpu1.regs[30][cpu1.rgs]+nn);
						 sprintf(buf, "%s %06X: %016I64X", kk == cpu1.regs[30][cpu1.rgs] ? "FP->" : "    ", (unsigned int)kk,
							 ((unsigned __int64)system1.Read(kk+4) << 32)|(unsigned __int64)system1.Read(kk));
						 this->listBoxBP->Items->Add(gcnew String(buf));
						 if (kk == cpu1.regs[30][cpu1.rgs])
							 this->listBoxBP->SetSelected(jj,true);
						 lastBP = cpu1.regs[30][cpu1.rgs];
						 jj++;
					 }
				 }
		 }
