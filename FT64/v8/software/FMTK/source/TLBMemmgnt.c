...
unsigned int GetMemKey()
{
	__asm {
		...
		shl		$r1,$r1,#1			; shift
		shr		$r2,$r1,#20			; extract bits 16 and 20
		shr		$t0,$r1,#16
		xor		$r2,$r2,$t0			; xor the bits
		and		$r2,$r2,#1			; we want just a single bit
...
private pascal int AllocPages(int npages, int key, int shared)
{
	...
	as = GetASID();
	if (shared)
		prevpage = virtpage = first_page = (shared_brks[as] >> 13) & 0xfffffffffff;
	else
		prevpage = virtpage = first_page = (brks[as] >> 13) & 0xfffffffffff;
	for (n = 0; n < npages; n++) {
		AllocPage(virtpage, key);
		virtpage++;
	}
	virtpage++;
	if (shared)
