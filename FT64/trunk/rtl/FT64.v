...
reg [5:0] rename_map [7:0][31:0];
reg [2:0] map_ndx;
reg [63:0] in_use [0:7];        // in use register map
reg [31:0] pc0;
reg [31:0] pc1;
// CSR's
reg [63:0] tick;
...
reg [7:0]  iqentry_sn   [0:7];  // instruction sequence number
...
wire [1:0] iqentry_islot [0:7];
assign iqentry_islot[0] = iqentry_0_islot;
assign iqentry_islot[1] = iqentry_1_islot;
assign iqentry_islot[2] = iqentry_2_islot;
assign iqentry_islot[3] = iqentry_3_islot;
assign iqentry_islot[4] = iqentry_4_islot;
assign iqentry_islot[5] = iqentry_5_islot;
assign iqentry_islot[6] = iqentry_6_islot;
assign iqentry_islot[7] = iqentry_7_islot;
...
always @*
if (irq_i > im && ~StatusHWI && ~int_commit)
	insn0 <= {17'd0,vec_i,`BRK};
else if (phit)
    insn0 <= insn0a;
else
    insn0 <= `NOP_INSN;
...
(
    .rst(rst),
    .wclk(clk),
    .wr((bstate==B2||(bstate==B1 && dhit0)) && ack_i),
    .sel(sel_o),
    .wadr({6'd0,adr_o}),
...
(
    .rst(rst),
    .wclk(clk),
    .wr((bstate==B2||(bstate==B1 && dhit1)) && ack_i),
    .sel(sel_o),
    .wadr({6'd0,adr_o}),
    .i(bstate==B2 ? dat_i : dat_o),
    .rclk(clk),
...
(
    .rst(rst),
    .wclk(clk),
    .wr((bstate==B2||(bstate==B1 && dhit2)) && ack_i),
    .sel(sel_o),
    .wadr({6'd0,adr_o}),
    .i(bstate==B2 ? dat_i : dat_o),
    .rclk(clk),
...
function IsAlu0Only;
input [31:0] isn;
IsAlu0Only = isn[`INSTRUCTION_OP]==`CSRRW;
endfunction
...
assign Rt0 = ffzo[6] ? 6'd0 : ffzo;
assign Rt1 = flzo[6] ? 6'd0 : flzo;
wire canq2 = Rt1!=Rt0 && (Rt1!=6'd0 && Rt0!=6'd0);
wire canq1 = (Rt1!=6'd0 || Rt0 != 6'd0);
    //
...
reg [1:0] s;
begin
s = iqentry_issue[a] + iqentry_issue[b];
fnSum2 = s[1] ? 2'b11 : s;
end
endfunction
function [1:0] fnSum3;
...
reg [1:0] s;
begin
s = iqentry_issue[a] + iqentry_issue[b] + iqentry_issue[c];
fnSum3 = s[1] ? 2'b11 : s;
end
endfunction
function [1:0] fnSum4;
...
reg [2:0] s;
begin
    s = iqentry_issue[a] + iqentry_issue[b] + iqentry_issue[c] + iqentry_issue[d];
    fnSum4 = |s[2:1] ? 2'b11 : s[1:0];
end
endfunction
function [1:0] fnSum5;
input [2:0] a;
...
reg [2:0] s;
begin
    s = iqentry_issue[a] + iqentry_issue[b] + iqentry_issue[c] + iqentry_issue[d] + iqentry_issue[e];
    fnSum5 = |s[2:1] ? 2'b11 : s[1:0];
end
endfunction
function [1:0] fnSum6;
input [2:0] a;
...
begin
    s = iqentry_issue[a] + iqentry_issue[b] + iqentry_issue[c] +
        iqentry_issue[d] + iqentry_issue[e] + iqentry_issue[f];
    fnSum6 = |s[2:1] ? 2'b11 : s[1:0];
end
endfunction
function [1:0] fnSum7;
input [2:0] a;
...
begin
    s = iqentry_issue[a] + iqentry_issue[b] + iqentry_issue[c] +
        iqentry_issue[d] + iqentry_issue[e] + iqentry_issue[f] +
        iqentry_issue[g];
    fnSum7 = |s[2:1] ? 2'b11 : s[1:0];
end
endfunction

    // Compute the issue slot
    assign iqentry_0_islot =
                  (head0 == 3'd1) ? fnSum7(1,2,3,4,5,6,7) : 
                  (head0 == 3'd2) ? fnSum6(2,3,4,5,6,7) : 
                  (head0 == 3'd3) ? fnSum5(3,4,5,6,7) : 
                  (head0 == 3'd4) ? fnSum4(4,5,6,7) : 
                  (head0 == 3'd5) ? fnSum3(5,6,7) : 
                  (head0 == 3'd6) ? fnSum2(6,7) : 
                  (head0 == 3'd7) ? {1'b0, iqentry_issue[7]} :
                  2'b00;

    assign iqentry_1_islot =
                  (head0 == 3'd2) ? fnSum7(2,3,4,5,6,7,0) : 
                  (head0 == 3'd3) ? fnSum6(3,4,5,6,7,0) : 
                  (head0 == 3'd4) ? fnSum5(4,5,6,7,0) : 
                  (head0 == 3'd5) ? fnSum4(5,6,7,0) : 
                  (head0 == 3'd6) ? fnSum3(6,7,0) : 
                  (head0 == 3'd7) ? fnSum2(7,0) : 
                  (head0 == 3'd0) ? {1'b0, iqentry_issue[0]} :
                  2'b00;

    assign iqentry_2_islot =
                  (head0 == 3'd3) ? fnSum7(3,4,5,6,7,0,1) : 
                  (head0 == 3'd4) ? fnSum6(4,5,6,7,0,1) : 
                  (head0 == 3'd5) ? fnSum5(5,6,7,0,1) : 
                  (head0 == 3'd6) ? fnSum4(6,7,0,1) : 
                  (head0 == 3'd7) ? fnSum3(7,0,1) : 
                  (head0 == 3'd0) ? fnSum2(0,1) : 
                  (head0 == 3'd1) ? {1'b0, iqentry_issue[1]} :
                  2'b00;

    assign iqentry_3_islot =
                  (head0 == 3'd4) ? fnSum7(4,5,6,7,0,1,2) : 
                  (head0 == 3'd5) ? fnSum6(5,6,7,0,1,2) : 
                  (head0 == 3'd6) ? fnSum5(6,7,0,1,2) : 
                  (head0 == 3'd7) ? fnSum4(7,0,1,2) : 
                  (head0 == 3'd0) ? fnSum3(0,1,2) : 
                  (head0 == 3'd1) ? fnSum2(1,2) : 
                  (head0 == 3'd2) ? {1'b0, iqentry_issue[2]} :
                  2'b00;

    assign iqentry_4_islot =
                  (head0 == 3'd5) ? fnSum7(5,6,7,0,1,2,3) : 
                  (head0 == 3'd6) ? fnSum6(6,7,0,1,2,3) : 
                  (head0 == 3'd7) ? fnSum5(7,0,1,2,3) : 
                  (head0 == 3'd0) ? fnSum4(0,1,2,3) : 
                  (head0 == 3'd1) ? fnSum3(1,2,3) : 
                  (head0 == 3'd2) ? fnSum2(2,3) : 
                  (head0 == 3'd3) ? {1'b0, iqentry_issue[3]} :
                  2'b00;

    assign iqentry_5_islot =
                  (head0 == 3'd6) ? fnSum7(6,7,0,1,2,3,4) : 
                  (head0 == 3'd7) ? fnSum6(7,0,1,2,3,4) : 
                  (head0 == 3'd0) ? fnSum5(0,1,2,3,4) : 
                  (head0 == 3'd1) ? fnSum4(1,2,3,4) : 
                  (head0 == 3'd2) ? fnSum3(2,3,4) : 
                  (head0 == 3'd3) ? fnSum2(3,4) : 
                  (head0 == 3'd4) ? {1'b0, iqentry_issue[4]} :
                  2'b00;

    assign iqentry_6_islot =
                  (head0 == 3'd7) ? fnSum7(7,0,1,2,3,4,5) : 
                  (head0 == 3'd0) ? fnSum6(0,1,2,3,4,5) : 
                  (head0 == 3'd1) ? fnSum5(1,2,3,4,5) : 
                  (head0 == 3'd2) ? fnSum4(2,3,4,5) : 
                  (head0 == 3'd3) ? fnSum3(3,4,5) : 
                  (head0 == 3'd4) ? fnSum2(4,5) : 
                  (head0 == 3'd5) ? {1'b0, iqentry_issue[5]} :
                  2'b00;

    assign iqentry_7_islot =
                  (head0 == 3'd0) ? fnSum7(0,1,2,3,4,5,6) : 
                  (head0 == 3'd1) ? fnSum6(1,2,3,4,5,6) : 
                  (head0 == 3'd2) ? fnSum5(2,3,4,5,6) : 
                  (head0 == 3'd3) ? fnSum4(3,4,5,6) : 
                  (head0 == 3'd4) ? fnSum3(4,5,6) : 
                  (head0 == 3'd5) ? fnSum2(5,6) : 
                  (head0 == 3'd6) ? {1'b0, iqentry_issue[6]} :
                  2'b00;
...
endgenerate
    ...
    assign iqentry_issue[0] = could_issue[0] 
				&& (head0 == 3'd0 || ~|iqentry_islot[7] ||
				    (iqentry_islot[7] == 2'b01 && ~iqentry_issue[7] && iqentry_instr[0][`INSTRUCTION_OP]!=`CSRRW));
    ...
    assign  alu0_misspc =
        ...
        (alu0_instr[`INSTRUCTION_OP] == `JAL) ? alu0_argA : (alu0_bt ? alu0_pc + 4 : alu0_pc + 4 + alu0_argI),
	    alu1_misspc = 
        (alu1_instr[`INSTRUCTION_OP] == `BRK) ? BRKPC :
	    (alu1_instr[`INSTRUCTION_OP] == `JAL) ? alu1_argA : (alu1_bt ? alu1_pc + 4 : alu1_pc + 4 + alu1_argI);
    ...
    assign alu0_branchmiss = alu0_dataready && 
			   ((alu0_instr[`INSTRUCTION_OP] == `BRK) ||
			    ((alu0_instr[`INSTRUCTION_OP] == `Bcc) ? ((|alu0_bus && ~alu0_bt) || (~|alu0_bus && alu0_bt))
			  : (alu0_instr[`INSTRUCTION_OP] == `JAL) ? (alu0_argB[`INSTRUCTION_S1] != `SYS_MFSR && alu0_argB[`INSTRUCTION_S1] != `SYS_MTSR)
			  : `INV));

    assign alu1_branchmiss = alu1_dataready && 
			   ((alu1_instr[`INSTRUCTION_OP] == `BRK) ||
			    ((alu1_instr[`INSTRUCTION_OP] == `Bcc)  ? ((|alu1_bus && ~alu1_bt) || (~|alu1_bus && alu1_bt))
              : (alu1_instr[`INSTRUCTION_OP] == `JAL) ? (alu1_argB[`INSTRUCTION_S1] != `SYS_MFSR && alu1_argB[`INSTRUCTION_S1] != `SYS_MTSR)
              : `INV));
    ...
    assign outstanding_stores = (dram0 && (dram0_op == `SW || dram0_op==`SH)) ||
                                (dram1 && (dram1_op == `SW || dram1_op==`SH)) ||
                                (dram2 && (dram2_op == `SW || dram2_op==`SH));
...
//
	...
	case ({fetchbuf0_v, fetchbuf1_v})
		...
		iqentry_bt   [tail0]    <=   (fetchbuf1_instr[`INSTRUCTION_OP] == `Bcc && predict_taken1); 
		...
		if (fetchbuf1_rfw) begin
		    for (n = 0; n < AREGS; n = n + 1)
		        rename_map[map_ndx+1][n] = rename_map[map_ndx][n];
		    rename_map[map_ndx+1][fnRt(fetchbuf1_instr)] <= Rt0;
		    in_use[map_ndx+1] <= in_use[map_ndx];
		    in_use[map_ndx+1][Rt0] <= 1'b1;
		    map_ndx <= map_ndx + 1;
    	    iqentry_map  [tail0]    <= map_ndx + 1;
    	    iqentry_fre  [tail0]    <= rename_map[map_ndx][fnRt(fetchbuf1_instr)];
		...
		iqentry_a1   [tail0]    <=   rf [rename_map[map_ndx][fnRa(fetchbuf1_instr)]];
		iqentry_a1_v [tail0]    <=   Source1Valid(fetchbuf1_instr) | rf_v[ rename_map[map_ndx][fnRa(fetchbuf1_instr)] ];
		iqentry_a1_s [tail0]    <=   rf_source [rename_map[map_ndx][fnRa(fetchbuf1_instr)]];
		iqentry_a2   [tail0]    <=    
					      ((fetchbuf1_instr[`INSTRUCTION_OP] == `JAL)
						 ? fetchbuf1_pc : rf[ rename_map[map_ndx][fnRb(fetchbuf1_instr)]]);
		iqentry_a2_v [tail0]    <=   Source2Valid( fetchbuf1_instr ) | rf_v[ rename_map[map_ndx][fnRb(fetchbuf1_instr)] ];
		iqentry_a2_s [tail0]    <=  rf_source[ rename_map[map_ndx][fnRb(fetchbuf1_instr)] ];
		tail0 <= tail0 + 1;
		tail1 <= tail1 + 1;
		seq_num <= seq_num + 8'd1;
		if (fetchbuf1_rfw) begin
		    rf_v[ Rt0 ] <= `INV;
		    rf_source[ Rt0 ] <= { fetchbuf1_mem, tail0 };	// top bit indicates ALU/MEM bus
		end
 
	    end

	    2'b10: if (queued1) begin
		if (fetchbuf0_instr[`INSTRUCTION_OP] != `Bcc)		panic <= `PANIC_FETCHBUFBEQ;
		...
		if (fetchbuf0_rfw) begin
            for (n = 0; n < AREGS; n = n + 1)
                rename_map[map_ndx+1][n] = rename_map[map_ndx][n];
            rename_map[map_ndx+1][fnRt(fetchbuf0_instr)] <= Rt0;
		    in_use[map_ndx+1] <= in_use[map_ndx];
            in_use[map_ndx+1][Rt0] <= 1'b1;
            map_ndx <= map_ndx + 1;
            iqentry_map  [tail0]    <= map_ndx + 1;
            iqentry_fre  [tail0]    <= rename_map[map_ndx][fnRt(fetchbuf0_instr)];
            iqentry_tgt  [tail0]    <= Rt0;
        end
        else begin
           iqentry_map  [tail0]    <= map_ndx;
		...
		iqentry_a1   [tail0]	<=	rf [rename_map[map_ndx][fnRa(fetchbuf0_instr)]];
		iqentry_a1_v [tail0]    <=	rf_v [rename_map[map_ndx][fnRa(fetchbuf0_instr)]];
		iqentry_a1_s [tail0]	<=	rf_source [rename_map[map_ndx][fnRa(fetchbuf0_instr)]];
		iqentry_a2   [tail0]	<=	rf[ rename_map[map_ndx][fnRb(fetchbuf0_instr) ]];
		iqentry_a2_v [tail0]    <=	rf_v[ rename_map[map_ndx][fnRb(fetchbuf0_instr) ]];
		iqentry_a2_s [tail0]	<=	rf_source[ rename_map[map_ndx][fnRb(fetchbuf0_instr) ]];
		tail0 <= tail0 + 1;
		tail1 <= tail1 + 1;
		seq_num <= seq_num + 8'd1;
		...
		//
		if ({fetchbuf0_instr[`INSTRUCTION_OP], predict_taken0} == {`Bcc, `TRUE}) begin
    		...
    		iqentry_a0   [tail0]    <=   assign_a0(tail0, fetchbuf0_instr);
		    iqentry_a1   [tail0]    <=   rf [rename_map[map_ndx][fnRa(fetchbuf0_instr)]];
		    iqentry_a1_v [tail0]    <=   Source1Valid( fetchbuf0_instr ) | rf_v[ rename_map[map_ndx][fnRa(fetchbuf0_instr) ]];
		    iqentry_a1_s [tail0]    <=   rf_source [rename_map[map_ndx][fnRa(fetchbuf0_instr)]];
		    iqentry_a2   [tail0]    <=   (fetchbuf0_instr[`INSTRUCTION_OP] == `JAL) ? fetchbuf0_pc
						                  : rf[ rename_map[map_ndx][fnRb(fetchbuf0_instr) ]];
		    iqentry_a2_v [tail0]    <=   Source2Valid( fetchbuf0_instr ) | rf_v[ rename_map[map_ndx][fnRb(fetchbuf0_instr) ]];
		    iqentry_a2_s [tail0]    <=   rf_source[ rename_map[map_ndx][fnRb(fetchbuf0_instr) ]];
			...
			iqentry_bt   [tail1]    <=   (fetchbuf1_instr[`INSTRUCTION_OP] == `Bcc 
    		...
    		if (fetchbuf1_rfw) begin
                for (n = 0; n < AREGS; n = n + 1)
                    rename_map[map_ndx+(fetchbuf0_rfw ? 2 : 1)][n] = rename_map[map_ndx][n];
        		if (fetchbuf0_rfw)
                    rename_map[map_ndx+2][fnRt(fetchbuf0_instr)] <= Rt0;
                rename_map[map_ndx+(fetchbuf0_rfw ? 2 : 1)][fnRt(fetchbuf1_instr)] <= Rt1;
    		    in_use[map_ndx+(fetchbuf0_rfw ? 2 : 1)] <= in_use[map_ndx];
                in_use[map_ndx+(fetchbuf0_rfw ? 2 : 1)][Rt1] <= 1'b1;
                map_ndx <= map_ndx + (fetchbuf0_rfw ? 2 : 1);
                iqentry_map  [tail1]    <= map_ndx + (fetchbuf0_rfw ? 2 : 1);
                iqentry_fre  [tail1]    <= rename_map[map_ndx][fnRt(fetchbuf1_instr)];
                iqentry_tgt  [tail1]    <= Rt1;
            end
            else begin
               iqentry_map  [tail1]    <= map_ndx + fetchbuf0_rfw + fetchbuf1_rfw;
            ...
            if (fetchbuf0_rfw && fnRa(fetchbuf1_instr)==fnRt(fetchbuf0_instr))
                iqentry_a1[tail1]   <=   rf [Rt0];
            else
			    iqentry_a1   [tail1]    <=   rf [rename_map[map_ndx][fnRa(fetchbuf1_instr)]];    // use outdated map
            if (fetchbuf0_rfw && fnRb(fetchbuf1_instr)==fnRt(fetchbuf0_instr))
                iqentry_a2[tail1]   <=   (fetchbuf1_instr[`INSTRUCTION_OP] == `JAL) ? fetchbuf1_pc : rf [Rt0];
            else
			    iqentry_a2   [tail1]    <=   (fetchbuf1_instr[`INSTRUCTION_OP] == `JAL) ? fetchbuf1_pc
							             : rf[ rename_map[map_ndx][fnRb(fetchbuf1_instr) ]];
			...
			// if previous instruction writes nothing to RF, then get info from rf_v and rf_source
			    ...
			    iqentry_a1_v [tail1]    <=   rf_v [rename_map[map_ndx][fnRa(fetchbuf1_instr)]];
			    iqentry_a1_s [tail1]    <=   rf_source [rename_map[map_ndx][fnRa(fetchbuf1_instr)]];
			end
			// otherwise, previous instruction does write to RF ... see if overlap
			// :Rt0 is newly assigned with register renaming, it can't match Ra.
//			else if (rename_map[map_ndx][fnRa(fetchbuf1_instr)] == Rt0) begin
...
//			end
			...
			// otherwise, previous instruction does write to RF ... see if overlap
//			else if (rename_map[map_ndx][fnRb(fetchbuf1_instr)] == Rt0) begin
//			    // if the previous instruction is a LW, then grab result from memq, not the iq
...
//    always @(posedge clk) begin: dataincoming_phase
	...
	if (alu0_v) begin
	    ...
	    $display("IsMem:%h", IsMem(iqentry_instr[ alu0_id[2:0] ]));
	    $display("!IsMem:%h", !IsMem(iqentry_instr[ alu0_id[2:0] ]));
	    iqentry_done[ alu0_id[2:0] ] <= !IsMem(iqentry_instr[ alu0_id[2:0] ]);
	...
	if (alu1_v) begin
	    ...
	    iqentry_done[ alu1_id[2:0] ] <= !IsMem(iqentry_instr[ alu1_id[2:0] ]);
...
//	if (dram2 != `DRAMSLOT_AVAIL)	dram2 <= dram2 + 2'd1;
	...
	casex ({dram0, dram1, dram2})
		...
		if (dram0 == `DRAMREQ_READY) begin
		    dram0 <= `DRAMSLOT_AVAIL;
		    dram_v <= IsLoad(dram0_op);
		    dram_id <= dram0_id;
		    dram_tgt <= dram0_tgt;
		    dram_exc <= dram0_exc;
		    case(dram0_op)
		    `LH:  dram_bus <= dram0_addr[0] ? {{32{rdat0[63]}},rdat0[63:32]} : {{32{rdat0[31]}},rdat0[31:0]};
		    `LHU: dram_bus <= dram0_addr[0] ? {{32{1'b0}},rdat0[63:32]} : {{32{1'b0}},rdat0[31:0]};
		    `LW:  dram_bus <= rdat0;
		    `SH:  ;
		    `SW:  ;
		    default: panic <= `PANIC_INVALIDMEMOP;
		    endcase
		    if (dram0_op == `SW || dram0_op==`SH) 	$display("m[%h] <- %h", dram0_addr, dram0_data);
		end
		else if (dram1 == `DRAMREQ_READY) begin
		    dram1 <= `DRAMSLOT_AVAIL;
		    dram_v <= IsLoad(dram1_op);
		    dram_id <= dram1_id;
		    dram_tgt <= dram1_tgt;
		    dram_exc <= dram1_exc;
		    case(dram1_op)
            `LH:  dram_bus <= dram1_addr[0] ? {{32{rdat1[63]}},rdat1[63:32]} : {{32{rdat1[31]}},rdat1[31:0]};
            `LHU: dram_bus <= dram1_addr[0] ? {{32{1'b0}},rdat1[63:32]} : {{32{1'b0}},rdat1[31:0]};
            `LW:  dram_bus <= rdat1;
            `SH:  ;
            `SW:  ;
            default: panic <= `PANIC_INVALIDMEMOP;
            endcase
            if (dram1_op == `SW || dram1_op==`SH)     $display("m[%h] <- %h", dram1_addr, dram1_data);
		end
		else if (dram2 == `DRAMREQ_READY) begin
		    dram2 <= `DRAMSLOT_AVAIL;
		    dram_v <= IsLoad(dram2_op);
		    dram_id <= dram2_id;
		    dram_tgt <= dram2_tgt;
		    dram_exc <= dram2_exc;
		    case(dram2_op)
            `LH:  dram_bus <= dram2_addr[0] ? {{32{rdat2[63]}},rdat2[63:32]} : {{32{rdat2[31]}},rdat2[31:0]};
            `LHU: dram_bus <= dram2_addr[0] ? {{32{1'b0}},rdat2[63:32]} : {{32{1'b0}},rdat2[31:0]};
            `LW:  dram_bus <= rdat2;
            `SH:  ;
            `SW:  ;
            default: panic <= `PANIC_INVALIDMEMOP;
            endcase
            if (dram2_op == `SW || dram2_op==`SH)     $display("m[%h] <- %h", dram2_addr, dram2_data);
		end
		else begin
		    dram_v <= `INV;
		end
    ...
    for (n = 0; n < QENTRIES; n = n + 1)
        if (~iqentry_stomp[n] && iqentry_memissue[n] && iqentry_agen[n] && ~iqentry_out[n]) begin
            ...
            dram0 		<= 2'd1;
            dram0_id 	<= { 1'b1, n[2:0] };
            dram0_op 	<= iqentry_instr[n][`INSTRUCTION_OP];
            ...
            dram1 		<= 2'd1;
            dram1_id 	<= { 1'b1, n[2:0] };
            dram1_op 	<= iqentry_instr[n][`INSTRUCTION_OP];
            ...
            dram2 		<= 2'd1;
            dram2_id 	<= { 1'b1, n[2:0] };
            dram2_op 	<= iqentry_instr[n][`INSTRUCTION_OP];
    ...
    begin
        if (IsImmp(iqentry_instr[n]) && iqentry_v[(n+1)&7]
            && (iqentry_sn[(n+1)&7]==iqentry_sn[n]+8'd1))
        iqentry_done[n] <= TRUE;
        if (!iqentry_v[n])
...
//    always @(posedge clk) begin: commit_phase
	...
	if (commit0_v) begin
        for (i = 0; i < 8; i = i + 1)
            in_use[i][iqentry_fre[head0]] <= 1'b0;
    end
	if (commit1_v) begin
        for (i = 0; i < 8; i = i + 1)
            in_use[i][iqentry_fre[head1]] <= 1'b0;
    end
...
IC3:    icstate <= IC4;
        ...
        // L2 cache. 
IC4:    if (ihit2 && (bstate==B11||picstate==IC3)) begin
            L1_wr1 <= TRUE;
            L1_wr0 <= TRUE;
            L1_adr <= L2_adr;
            L2_rdat <= L2_dato;
...
BIDLE:
    begin
        ...
        if (dram0==2'd1 && (dram0_op==`SW || dram0_op==`SH)) begin
            ...
            `SH:    begin
                        we_o <= `HIGH;
                        sel_o <= dram0_addr[0] ? 2'b10 : 2'b01;
                        adr_o <= dram0_addr;
                        dat_o <= {2{dram0_data[31:0]}};
                        bstate <= B13;
                    end
            endcase
        end
        else if (dram1==2'd1 && (dram1_op==`SW || dram1_op==`SH)) begin
            dram1 <= 2'd2;
            bwhich <= 2'b01;
            case(dram1_op)
            `SW:    begin
            ...
            `SH:    begin
                        we_o <= `HIGH;
                        sel_o <= dram1_addr[0] ? 2'b10 : 2'b01;
                        adr_o <= dram1_addr;
                        dat_o <= {2{dram1_data[31:0]}};
                        bstate <= B13;
                    end
            endcase
        end
        else if (dram2==2'd1 && (dram2_op==`SW || dram2_op==`SH)) begin
            dram2 <= 2'd2;
            bwhich <= 2'b10;
            case(dram2_op)
            `SW:    begin
            ...
            `SH:    begin
                        we_o <= `HIGH;
                        sel_o <= dram2_addr[0] ? 2'b10 : 2'b01;
                        adr_o <= dram2_addr;
                        dat_o <= {2{dram2_data[31:0]}};
                        bstate <= B13;
                    end
            endcase
        end
        // Check for read misses on the data cache
        else if (!dram0_unc && dram0==2'd1 && !dhit0 && IsLoad(dram0_op)) begin
            dram0 <= 2'd2;
            bwhich <= 2'b00;
            cyc_o <= `HIGH;
            stb_o <= `HIGH;
        ...
        else if (!dram1_unc && dram1==2'd1 && !dhit1 && IsLoad(dram1_op)) begin
            dram1 <= 2'd2;
            bwhich <= 2'b01;
            cyc_o <= `HIGH;
            stb_o <= `HIGH;
        ...
        else if (!dram2_unc && dram2==2'd1 && !dhit2 && IsLoad(dram2_op)) begin
            dram2 <= 2'd2;
            bwhich <= 2'b10;
            cyc_o <= `HIGH;
            stb_o <= `HIGH;
        ...
        else if (dram0_unc && dram0==2'd1 && IsLoad(dram0_op)) begin
            bwhich <= 2'b00;
            cyc_o <= `HIGH;
            stb_o <= `HIGH;
            sel_o <= 2'b11;
        ...
        else if (dram1_unc && dram1==2'd1 && IsLoad(dram1_op)) begin
            bwhich <= 2'b01;
            cyc_o <= `HIGH;
            stb_o <= `HIGH;
            sel_o <= 2'b11;
        ...
        else if (dram2_unc && dram2==2'd1 && IsLoad(dram2_op)) begin
            bwhich <= 2'b10;
            cyc_o <= `HIGH;
            stb_o <= `HIGH;
            sel_o <= 2'b11;
...
//	#5 rf[0] = 0; rf_v[0] = 1; rf_source[0] = 0;
	...
	for (i=0; i<8; i=i+1) 
	    $display("%c%c %d: %d %d %d %d %d %d %d %d %d %c%h 0%d(%d) %o %h %h %h %d %o %h %d %o %h #",
	...
	$display("%d %h %h %c%d %o #",
	    dram0, dram0_addr, dram0_data, (IsFlowCtrl(dram0_op) ? 98 : (IsMem({21'b0,dram0_op})) ? 109 : 97), 
	    dram0_op, dram0_id);
	$display("%d %h %h %c%d %o #",
	    dram1, dram1_addr, dram1_data, (IsFlowCtrl(dram1_op) ? 98 : (IsMem({21'b0,dram1_op})) ? 109 : 97), 
	    dram1_op, dram1_id);
	$display("%d %h %h %c%d %o #",
	    dram2, dram2_addr, dram2_data, (IsFlowCtrl(dram2_op) ? 98 : (IsMem({21'b0,dram2_op})) ? 109 : 97), 
	    dram2_op, dram2_id);
...
begin
    if (v) begin
        ...
        `RR:
            ...
            `SEI:   im <= iqentry_instr[head][23:21];   // S1
            `CLI:   im <= 3'd0;
            `RTI:   begin
                    im <= mstatus[2:0];
