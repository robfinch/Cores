...
input [63:0] data64;
case(opcode)
`LW,`LM,`LFD,`LSW,`LP,`LFDP,`LWR,
`INW:	fnDataIn = data64;
`LH,`LF,`LFP,
`INH:	fnDataIn = {{32{data32[31]}},data32};
`LHU,`LSH,`INHU:	fnDataIn = data32;
`LC,`INCH:	fnDataIn = {{48{data16[15]}},data16};
`LCU,`INCU:	fnDataIn = data16;
`LB,`INB:	fnDataIn = {{56{data8[7]}},data8};
`LBU,`INBU:	fnDataIn = data8;
default:	fnDataIn = 64'h0;
endcase
endfunction
...
// Segment register #15 is designated as the code segment (CS). The program
// counter always references seg#15.
//
// There is a non-segmented code address range at 64'hFxxxxxxxxxxxxxxx
// This range allows the operating system to work without paying attention
// to the current code segment. The only way to change the code segment
// is by transferring to the operating system via a syscall.
//-----------------------------------------------------------------------------
`ifdef SEGMENTATION
...
wire [63:12] sego1;
wire [63:12] sego2;
wire wIsMtseg = (wOpcode==`R && wFunc==`MTSEG) || (wOpcode==`RR && wFunc6==`MTSEGI);
wire wr_segs = advanceW & wIsMtseg;
...
assign spc = pc[63:60]==4'hf ? pc : {sego0[63:12] + pc[63:12],pc[11:0]};
assign sea = {sego1[63:12] + ea[59:12],ea[11:0]};
`else
wire [63:0] spc = pc;
wire [63:0] sea = ea;
`endif
...
// SimpleMMU
//
// Divides a 128MB address space up into 512, 256kB pages. The page tables
// are stored directly in the MMU. Up to 32 tasks are supported with a single
// MMU.
//-----------------------------------------------------------------------------
`ifdef SIMPLE_MMU
SimpleMMU ummu1
...
wire xIsStore = (
	...
	xOpcode==`SSH || xOpcode==`SSW || xOpcode==`STBC || xOpcode==`SWU ||
	(xOpcode==`MEMNDX && (
...
wire StallR =  	((( xIsLoad||xIsIn||xIsCnt) &&   xForwardingActive) || xIsSWC) ||
				...
				;
wire StallX = ((xneedBus||xIsLoad||xIsStore) & (m1needBus|m2needBus|icaccess|iocyc_o|cyc_o));
...
$display("iocyc=%d  cyc=%d stb_o=%d sel_o=%b", iocyc_o, cyc_o, stb_o, sel_o);
prev_nmi <= nmi_i;
if (!prev_nmi & nmi_i)
	nmi_edge <= 1'b1;
...
`endif
	...
	//			end
		...
		`SW,`SH,`SC,`SB,`SWC,`SF,`SFD,`SM,`SSW,`SP,`SFP,`SFDP,`SWU:
				begin
				m1DataOut <= b;
				ea <= xData;
	...
	//			end
		...
		`MEMNDX:
				...
				`INWX,`INHX,`INHUX,`INCX,`INCUX,`INBX,`INBUX:
						begin
						iocyc_o <= 1'b1;
						stb_o <= 1'b1;
						sel_o <= fnSelect(7'd32 + xFunc6,xData[2:0]);
						adr_o <= xData;
						end
				`OUTWX,`OUTHX,`OUTCX,`OUTBX:
						begin
						...
						sel_o <= fnSelect(7'd32 + xFunc6,xData[2:0]);
						adr_o <= xData;
						dat_o <= fnDataOut(7'd32 + xFunc6,c);
						end
				default:
					begin
					m1DataOut <= c;
...
if (advanceM1) begin
	...
	`INW,`INH,`INHU,`INCH,`INCU,`INB,`INBU:
		begin
			...
			stb_o <= 1'b0;
			sel_o <= 8'h00;
			m2Data <= fnDataIn(m1Opcode,data8,data16,data32,data64);
		end
	...
	`LC,`LCU,`LB,`LBU:
		if (!m1IsCacheElement) begin
			...
			m2IsLoad <= 1'b0;
			m2IR <= `NOP_INSN;
			m2Data <= fnDataIn(m1Opcode,cdata8,cdata16,cdata32,cdata64);
		end
`ifdef ADDRESS_RESERVATION
	`LWR:
		begin
...
`endif
			...
			stb_o <= #1 1'b1;
			we_o <= #1 1'b1;
			sel_o <= #1 fnSelect(m1Opcode,pea[2:0]);
			adr_o <= #1 pea;
			dat_o <= #1 fnDataOut(m1Opcode,m1DataOut);
		end
...
if (advanceM2) begin
	...
	`SH,`SC,`SB,`SW,`SWC,`SF,`SFD,`SSH,`SSW,`SP,`SFP,`SFDP,`SWU:
		begin
			cyc_o <= #1 1'b0;
			stb_o <= #1 1'b0;
			we_o <= #1 1'b0;
	...
	`LHU,`LC,`LCU,`LB,`LBU:
		begin
			...
			stb_o <= #1 1'b0;
			sel_o <= #1 8'h00;
			wData <= #1 fnDataIn(m2Opcode,data8,data16,data32,data64);
		end
	default:	;
	endcase
...
module seg_regfile(clk, wr, wa, i, ra0, ra1, ra2, o0, o1, o2);
input clk;
input wr;
input [7:0] wa;
input [63:12] i;
