`timescale 1ns / 1ps
//=============================================================================
//	(C) 2007,2010,2012,2013  Robert Finch
...
module PSG16(rst_i, clk_i, cyc_i, stb_i, ack_o, we_i, adr_i, dat_i, dat_o,
	vol_o, bg, 
...
output ack_o;
input we_i;				// write
input [33:0] adr_i;		// address input
...
output [33:0] m_adr_o;	// wave table address
...
wire cs = cyc_i && stb_i && (adr_i[33:10]==24'hFFD5_00);
...
begin
	if (rst_i) begin
		...
		ringmod <= 0;
		volume <= 0;
		m_adr_o[33:14] <= 34'b00_0000_0000_0000_0011_10;	// 00038000
	end
	else begin
		if (cs & we_i) begin
			case(adr_i[8:2])
			...
			//---------------------------------------------------------
			7'd64:	volume <= dat_i[3:0];

			7'd68:	begin
						m_adr_o[15:14] <= dat_i[15:14];
					end
			7'd69:
					begin
						m_adr_o[31:16] <= dat_i[15:0];
					end
			7'd70:	begin
						m_adr_o[33:32] <= dat_i[ 1:0];
					end
			default:	;
			endcase
		end
