...
static void process_addi(int funct3)
{
    ...
    if ((val < -2048 || val > 2047)) {
      if (OPTX32) {
		...
		if (val < -2048 || val > 2047) {
			if (OPT_NSI) {
				emit_insn((0x800 << 20) | RS1(Ra) | FN3(funct3) | RD(Rt) | 0x13, !expand_flag);
...
static void process_riop(int oc, int funct3, int funct7)
{
  ...
  if (!IsNBit(val,12)) {
		if (OPT_NSI) {
			emit_insn((0x800 << 20LL) | FN3(funct3) | RS1(Ra) | RD(Rt) | oc, !expand_flag);  // ADDI
			emit_insn(val, !expand_flag);
			return;
		}
		if (val & 0x800LL)
  ...
  if ((val < -2048 || val > 2047)) {
    ...
    emit_insn(((val & 0xFFFLL) << 20LL)|RS1(Ra)|FN3(funct3)|RD(Rt)|oc,!expand_flag);
...
static void process_rrop(int oc, int funct3, int funct7)
{
  ...
  if (token=='#') {
    ...
    case tk_add: process_riop(0x13,0x00,funct7); break;
		...
		case tk_gcsub: process_riop(-13, 3, 0); break;
...
j2:
				if (val < -2048 || val > 2047) {
					if (OPT_NSI) {
						emit_insn((0x800 << 20) | RS1(Ra) | RD(Rt) | 0x67, 1);
						emit_insn(val, 1);
						return;
					}
    ...
    if (disp > 524287 || disp < -524288) {
		...
		val =
			(((disp & 0x100000LL) >> 20LL) << 31LL) |
			(((disp & 0x7FELL) >> 1LL) << 21LL) |
			(((disp & 0x800LL) >> 11LL) << 20LL) |
			(((disp & 0xFF000LL) >> 12LL) << 12LL);
		emit_insn(
      val |
      RD(Rt) |
      0x6F, 0
    );
//		if (((val >> 12LL) & 0xfffLL)==0x800LL) {
//			emit_insn(disp>>1, 0);
//		}
}
...
static void process_store(int oc, int func3)
{
    ...
    if (Ra < 0) Ra = 0;
		...
		*/
		if ((disp < -2048 || disp > 2047)) {
			if (OPT_NSI) {
				emit_insn((0x40 << 25) | RD(0x00) | RS2(Rs) | RS1(Ra) | FN3(func3) | oc, !expand_flag);
				emit_insn(disp, !expand_flag);
				return;
			}
			else {
				if (disp & 0x800)
...
//        printf("Line:%.60s\r\n",p);
    ...
    if (Ra < 0) Ra = 0;
		if ((disp < -2048 || disp > 2047)) {
			if (OPT_NSI) {
				emit_insn((0x800 << 20) | RS1(Ra) | FN3(func3) | RD(Rt) | oc, !expand_flag);
				emit_insn(disp, !expand_flag);
				return;
			}
			else {
...
//        case tk_add:  process_add(); break;
        ...
        case tk_align: process_align(); continue; break;
				case tk_gcsub:  process_rrop(13, 0, 32); break;
        ...
        case tk_bgeu: process_bcc(7); break;
				...
				case tk_bgtz: process_beqz(-4); break;
...
//				case tk_fsq:	process_store(39, 4); break;
        ...
        case tk_gran: process_gran(0x14); break;
				case tk_getrdy: process_rrop(13, 0, 14); break;
...
//				case tk_ld: process_ldi(); break;
        ...
        case tk_mov: process_mov(0x04); break;
				case tk_mret: emit_insn(0x30200073, 1); break;
        ...
        case tk_org: process_org(); break;
				...
				case tk_peekq: process_getto(13, 0, 14); break;
				case tk_pfi: emit_insn(0x10300073, 1); break;
				case tk_pfree: process_palloc(5); break;
				case tk_php: emit_insn(0x3200000001,1); break;
        case tk_plp: emit_insn(0x3300000001,1); break;
        case tk_plus: expand_flag = 1; break;
				case tk_popq: process_getto(13, 0, 13); break;
				case tk_pstat: process_rrop(13, 0x00, 0x07); break;
				case tk_public: process_public(); break;
				case tk_pushq: process_setto(13, 0, 12); break;
