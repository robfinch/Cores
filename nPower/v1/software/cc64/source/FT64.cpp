	...
	if (op==op_fbne || op==op_fbeq || op==op_fblt || op==op_fble || op==op_fbgt || op==op_fbge) {
		...
		case op_bor:
			if (ap2->mode == am_imm) {
				...
				GenerateDiadic(op_bor, 0, ap2, MakeCodeLabel(label));
			break;
		case op_bnand:
			if (ap2->mode == am_imm) {
				ap3 = GetTempRegister();
				...
				GenerateDiadic(op_bnand, 0, ap2, MakeCodeLabel(label));
			}
			break;
		case op_bnor:
			if (ap2->mode == am_imm) {
				...
				GenerateDiadic(op_bnor, 0, ap2, MakeCodeLabel(label));
			break;
		case op_beq:
			if (ap2->mode==am_imm) {
...
static void SaveRegisterSet(SYM *sym)
{
	...
	if (!cpu.SupportsPush) {
		...
		for (nn = 1 + (sym->tp->GetBtp()->type!=bt_void ? 1 : 0); nn < 31; nn++) {
			GenerateDiadic(op_sth,0,makereg(nn),cg.MakeIndexed(mm,regSP));
			mm += sizeOfWord;
		}
	}
	else
...
static void RestoreRegisterSet(SYM * sym)
{
	...
	if (!cpu.SupportsPop) {
		mm = 0;
		for (nn = 1 + (sym->tp->GetBtp()->type!=bt_void ? 1 : 0); nn < 31; nn++) {
			GenerateDiadic(op_ldh,0,makereg(nn),cg.MakeIndexed(mm,regSP));
			mm += sizeOfWord;
		}
		mm = sym->tp->GetBtp()->type!=bt_void ? 29 : 30;
...
void SaveRegisterVars(CSet *rmask)
{
	...
	if( rmask->NumMember() ) {
		...
		for (nn = rmask->lastMember(); nn >= 0; nn = rmask->prevMember()) {
			...
			// regno = -(nn - nregs + 1);
			// regno = nregs - 1 - nn
			GenerateDiadic(op_sth,0,makereg(nregs-1-nn),cg.MakeIndexed(cnt,regSP));
			cnt+=sizeOfWord;
		}
	}
}
...
static void RestoreRegisterVars()
{
	...
	if( save_mask->NumMember()) {
		...
		for (nn = save_mask->nextMember(); nn >= 0; nn = save_mask->nextMember()) {
			GenerateDiadic(op_ldh,0,makereg(nn),cg.MakeIndexed(cnt,regSP));
			cnt += sizeOfWord;
		}
				...
				if (cpu.SupportsPush) {
					...
					if (ap->mode==am_imm) {	// must have been a zero
						...
						if (ap->offset->i!=0) {
							...
							GenLoadConst(ap, ap3);
	         				GenerateDiadic(op_sth,0,ap3,MakeIndexed(stkoffs,regSP));
							ReleaseTempReg(ap3);
						}
						else {
							GenerateDiadic(op_sth, 0, makereg(0), MakeIndexed(stkoffs, regSP));
						}
						nn = 1;
					}
					else {
						if (ap->type==stddouble.GetIndex() || ap->mode==am_fpreg) {
							*isFloat = true;
							...
							regs[ap->preg].IsArg = true;
							GenerateDiadic(op_sth,0,ap,MakeIndexed(stkoffs,regSP));
							nn = 1;
						}
					}
				}
