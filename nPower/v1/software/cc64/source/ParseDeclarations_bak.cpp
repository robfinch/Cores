...
// ()
SYM *Declaration::ParsePrefixOpenpa(bool isUnion, SYM* parent)
{
...
SYM *Declaration::ParsePrefix(bool isUnion, SYM* parent)
{   
	TYP *temp1;
	SYM *sp;
...
//		case ellipsis:
	...
	case openpa:
		sp = ParsePrefixOpenpa(isUnion, parent);
		goto lxit;

	default:
		sp = ParseSuffix(parent);
		dfs.printf("Z");
		goto lxit;
	}
lxit:
 ...
 // currentFn = sp;
	...
	if (parsingParameterList || isStructDecl) {
		...
		if (nump == -1)
			...
			pd.ParameterDeclaration::Parse(1);
		//currentFn = cf;
		needParseFunction = 0;
		dfs.printf("Set false\n");
		//				sp->parms = sym;
...
SYM* Declaration::ParseSuffixOpenpa(SYM *parent)
{
	TYP *temp1, *temp2;
	TYP *tempHead, *tempTail;
	int fd;
	...
	funcdecl++;
	//sp->fi->params.Clear();
	sp->fi->sym->parent = currentClass->GetIndex();
	if(lastst == closepa) {
		funcdecl--;
		NextToken();
		while (lastst == kw_attribute)
			ParseFunctionAttribute(sp->fi);
	  if(lastst == begin) {
		  temp1->type = bt_ifunc;
			needParseFunction = 2;
			//sp->fi->NumParms = 0;
  ...
  int nbytes;
	...
	for(;;) {
	  if (declid) delete declid;
		...
		dfs.printf("b");
		bit_width = -1;
		sp = ParsePrefix(ztype==bt_union, parent);
