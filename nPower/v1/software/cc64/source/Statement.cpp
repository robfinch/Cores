...
Statement *Statement::ParseFor()
{
	...
	if ((iflevel > maxPn - 1) && isThor)
		error(ERR_OUTOFPREDS);
	needpunc(openpa, 16);
//	expression(&(snp->initExpr), nullptr);
...
//	expression(&(snp->incrExpr), nullptr);
...
void Statement::GenerateWhile()
{
	...
	if (s1 != NULL)
	{
		...
		if (!opt_nocgo && !opt_size) {
			...
			GenerateMonadic(cpu.bra_op, 0, cg.MakeCodeLabel(contlab));
	}
	else
	{
		initstack();
...
void Statement::GenerateFor()
{
	...
	if (opt_nocgo||opt_size)
		GenerateMonadic(cpu.bra_op, 0, cg.MakeCodeLabel(loop_label));
	else {
		initstack();
...
void Statement::GenerateDoLoop()
{
	...
	looplevel--;
	GenMixedSource2();
	GenerateMonadic(cpu.bra_op, 0, MakeCodeLabel(contlab));
	GenerateLabel(breaklab);
	currentFn->pl.OptLoopInvariants(loophead);
	breaklab = oldbreak;
	contlab = oldcont;
...
//	GenerateMonadic(op_push, 0, ap);	// stack it
	sprintf_s(buf, sizeof(buf), ".C%05lld@ha", (int64_t)throwlab);
	GenerateDiadic(op_lis, 0, ap, MakeStringAsNameConst(buf, codeseg));
	DataLabels[throwlab] = true;
	sprintf_s(buf, sizeof(buf), ".C%05lld@l", (int64_t)throwlab);
	GenerateTriadic(op_addi, 0, ap, ap, MakeStringAsNameConst(buf, codeseg));
	if (currentFn->IsFar)
		GenerateMonadic(op_di, 0, MakeImmediate(2));
	GenerateDiadic(cpu.stt_op, 0, ap, MakeIndexed(4 * sizeOfWord, regFP));	// Set current handler
	if (currentFn->IsFar)
		GenerateDiadic(cpu.stt_op, 0, makereg(regCS), MakeIndexed((int64_t)5 * sizeOfWord, regFP));	// Set current handler
	if (compiler.ipoll)
		GenerateZeradic(op_pfi);
...
void Statement::GenerateThrow()
{
	...
	ap = GetTempRegister();
	GenerateMonadic(cpu.bra_op, 0, MakeCodeLabel(throwlab));
	ReleaseTempRegister(ap);
}
...
void Statement::GenerateCatch(int opt, int oldthrow, int olderthrow)
{
	...
	ap = GetTempRegister();
	sprintf_s(buf, sizeof(buf), ".C%05lld@ha", (int64_t)oldthrow);
	GenerateDiadic(op_lis, 0, ap, MakeStringAsNameConst(buf, codeseg));
	DataLabels[oldthrow] = true;
	sprintf_s(buf, sizeof(buf), ".C%05lld@l", (int64_t)oldthrow);
	GenerateTriadic(op_addi, 0, ap, ap, MakeStringAsNameConst(buf, codeseg));
	if (currentFn->IsFar)
		GenerateMonadic(op_di, 0, MakeImmediate(2));
	GenerateDiadic(cpu.stt_op, 0, ap, MakeIndexed((int64_t)4 * sizeOfWord, regFP));	// Restore handler
	if (currentFn->IsFar)
		GenerateDiadic(cpu.stt_op, 0, makereg(regCS), MakeIndexed((int64_t)5 * sizeOfWord, regFP));	// Restore handler
	ReleaseTempRegister(ap);
	// Branch around the catch handlers
	if (opt == 0) {
		GenerateMonadic(cpu.bra_op, 0, MakeCodeLabel(lab1));
	...
	for (stmt = opt ? this : s2; stmt; stmt = stmt->next) {
		...
		if (stmt->num == 99999 && opt==1)
			...
			GenerateMonadic(cpu.bra_op, 0, MakeCodeLabel(opt ? retlab : lab1));
	...
	for (stmt = this; stmt != NULL; stmt = stmt->next)
	{
		...
		*/
		...
		case st_label:
			...
			break;
		case st_goto:
			GenerateMonadic(cpu.bra_op, 0, MakeCodeLabel((int64_t)stmt->label));
			break;
			//case st_critical:
		...
		case st_continue:
			...
			GenerateDiadic(cpu.bra_op, 0, MakeCodeLabel(contlab), 0);
			break;
		case st_break:
			if (breaklab == -1)
				error(ERR_NOT_IN_LOOP);
			GenerateDiadic(cpu.bra_op, 0, MakeCodeLabel(breaklab), 0);
			break;
		case st_switch:
			stmt->GenerateSwitch();
			break;
