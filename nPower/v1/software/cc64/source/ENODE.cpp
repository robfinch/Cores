...
j1:
	...
	if (n1 > 4 || currentFn->pl.HasCall(ip1))
	{
		...
		GenerateDiadic(cpu.bra_op, 0, MakeCodeLabel(end_label), 0);
...
Operand *ENODE::GenerateShift(int flags, int size, int op)
{
	...
	case op_sra:
		switch (size) {
		case 4:	op = ap2->mode == am_reg ? op_sraw : op_srawi; break;
		case 8:	op = ap2->mode == am_reg ? op_srad : op_sradi; break;
		}
		break;
	case op_sll:
		switch (size) {
		case 4:	op = ap2->mode == am_reg ? op_slw : op_slwi; break;
		case 8:	op = ap2->mode == am_reg ? op_sld : op_sldi; break;
		}
		break;
	}
	...
	if (op == op_fdiv) {
		...
		if (op == op_rem) {
			if (ap2->mode == am_imm) {
				...
				ap4 = GetTempRegister();
				GenerateTriadic(op_divw, 0, ap4, ap1, ap2);
				GenerateTriadic(op_mullw, 0, ap4, ap4, ap2);
				GenerateTriadic(op_subf, 0, ap3, ap4, ap1);
				ReleaseTempReg(ap4);
			}
		}
	...
	else if (IsPositType())
	{
		...
		if (ENODE::IsEqual(p[0], p[1]) && !opt_nocgo) {
			...
			if (op == op_sub || op == op_ptrdif || op == op_eor)
				GenerateDiadic(op_li, 0, ap3, MakeImmediate(0));
			else {
				ap1 = cg.GenerateExpression(p[0], am_reg, size, 0);
				GenerateTriadic(op, 0, ap3, ap1, ap1);
			}
