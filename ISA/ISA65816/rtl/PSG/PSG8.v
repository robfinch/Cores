...
module PSG8(rst_i, ph2, clk50_i, cs_i, rw_i, adr_i, dat_i, dat_o, m_adr_o, m_dat_i, o);
parameter BUS_WID = 8;
// WISHBONE SYSCON
input rst_i;
input ph2;			// system bus clock
...
reg [8:0] radr;
always @(negedge ph2)
    if (cs & ~rw_i)  reg_shadow[adr_i[8:0]] <= dat_i;
always @(posedge ph2)
    radr <= adr_i[8:0];
...
reg [8:0] radr;
always @(negedge ph2)
    if (cs & ~rw_i)  reg_shadow[adr_i[8:2]] <= dat_i;
always @(posedge ph2)
    radr <= adr_i[8:2];
