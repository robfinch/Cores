...
parameter MDW = 104;		// Bus master data width
parameter MAP = 12'd0;
...
`endif
output [25:0] zrgb_o;		// 24-bit RGB output + z-order
reg [25:0] zrgb_o;
...
(
  ...
  .dina({6'd0,dat[25:0]}),    // input wire [31 : 0] dina
  ...
  if (rstcmd & ~rstcmd1)
	...
	if (cs_edge) begin
		if (we) begin
			...
			REG_WINDOW:
				begin
					windowWidth <= dat[11:0];
					windowHeight <= dat[24:13];
					windowLeft <= dat[37:26];
					windowTop <= dat[50:38];
				end
...
`ifdef INTERNAL_SYNC_GEN
			REG_TOTAL:
				begin
					...
					begin
						if (dat[51:26]==26'h1123456)
							sgLock <= 1'b0;
						else if (dat[51:26]==26'h1543211)
							sgLock <= 1'b1;
					end
				end
			REG_SYNC_ONOFF:
...
`endif
  ...
  REG_PXYZ:		    s_dat_o <= {13'h0,pz,py,px};
  REG_PCOLCMD:    s_dat_o <= {color_o,6'd0,raster_op,14'd0,pcmd};
  REG_OOB_COLOR:	s_dat_o <= {26'h0,oob_color};
  REG_WINDOW:			s_dat_o <= {windowTop,1'b0,windowLeft,1'h0,windowHeight,1'h0,windowWidth};
  9'b10??_????_?:	s_dat_o <= {26'h0,pal_wo[25:0]};
  default:        s_dat_o <= 52'd0;
  endcase
end
...
default:
	begin
	$display("BitmapController52: Bad master bus width");
	$finish;
	end
endcase
...
wire[51:0] grAddr,xyAddr;
reg [11:0] fetchCol;
localparam CMS = MDW==104 ? 6 : MDW==52 ? 5 : 4;
wire [CMS:0] mb,me,ce;
...
assign m_sel_o = MDW==104 ? 8'hFF : MDW==52 ? 4'hF : 2'h3;
...
always @*
	case(MDW)
	26:	modd <= m_adr_o[4:1]==4'hF;
	52:	modd <= m_adr_o[4:2]==3'h7;
	default:	modd <= m_adr_o[4:3]==2'h3;
	endcase
...
always @(posedge vclk)
case(color_depth)
BPP4:	rgbo4 <= {rgbo3[3],22'd0,rgbo3[2:0]};	// feeds into palette
BPP8:	rgbo4 <= {rgbo3[7:6],18'h0,rgbo3[5:0]};		// feeds into palette
BPP12:	rgbo4 <= {rgbo3[11:9],2'd0,rgbo3[8:6],4'd0,rgbo3[5:3],4'd0,rgbo3[2:0],4'd0};
BPP16:	rgbo4 <= {rgbo3[15:12],1'b0,rgbo3[11:8],3'b0,rgbo3[7:4],3'b0,rgbo3[3:0],3'b0};
BPP20:	rgbo4 <= {rgbo3[19:15],rgbo3[14:10],2'b0,rgbo3[9:5],2'b0,rgbo3[4:0],2'b0};
BPP26:	rgbo4 <= rgbo3;
default:	rgbo4 <= {rgbo3[11:9],2'd0,rgbo3[8:6],4'd0,rgbo3[5:3],4'd0,rgbo3[2:0],4'd0};
endcase
	...
	if (onoff && xonoff_i && !blank_i) begin
		if (color_depth[2:1]==2'b00) begin
			if (!greyscale)
				...
				zrgb_o <= {pal_o[25:21],{3{pal_o[6:0]}}};
		end
		else
			zrgb_o <= rgbo4;
	end
...
always @(posedge vclk)
	if (rd_fifo)
		...
		BPP26:	rgbo3 <= {26'h0,rgbo3[MDW-1:26]};
		default: rgbo3 <= {12'h0,rgbo3[MDW-1:12]};
		endcase
	end
...
BPP26:	oob_dat <= {MDW/26{oob_color[25:0]}};
default:	oob_dat <= {MDW/12{oob_color[11:0]}};
endcase
