...
module SpriteController52(clk_i, cs_i, cyc_i, stb_i, ack_o, we_i, sel_i, adr_i, dat_i, dat_o,
...
reg [51:0] m_spriteBmp [0:NSPR-1];
reg [51:0] spriteBmp [0:NSPR-1];
reg [NSPR-1:0] spriteLink1 = 32'h0;
reg [7:0] spriteColorNdx [0:NSPR-1];

initial begin
	for (n = 0; n < 256; n = n + 1) begin
		sprite_color[n] <= {23'h00,8'h00,n[7:5],4'd0,n[4:3],5'd0,n[2:0],4'd0};
...
(* ram_style="block" *)
reg [51:0] shadow_ram [0:511];
reg [51:0] shadow_ramo;
reg [8:0] sradr;
always @(posedge clk)
	if (cs & we) begin
		shadow_ram[adr[11:3]][51: 0] <= dat[51: 0];
	end
always @(posedge clk)
	sradr <= adr[11:3];
always @(posedge clk)
...
begin
	...
	if (cs & we) begin
		casez(adr[11:3])
		9'b0???_????_?:	sprite_color[adr[10:3]] <= dat[31:0];
		9'b10??_????_0:	spriteAddr[adr[9:4]] <= dat[`ABITS];
		9'b10??_????_1:
			begin
				sprite_ph[adr[9:4]] <= dat[12: 0];
				sprite_pv[adr[9:4]] <= dat[25:13];
				sprite_pz[adr[9:4]] <= dat[33:26];
				spriteMcnt[adr[9:4]] <= dat[49:34];
			end
		9'b1100_0000_0:	spriteEnable <= dat[NSPR-1:0];
		9'b1100_0000_1:	spriteLink1 <= dat[NSPR-1:0];
		9'b1100_0001_0:	rst_collision <= `TRUE;
		9'b1100_0001_1:
			begin
				lowres <= dat[1:0];
...
MEM_ACCESS:
	if (m_ack_i|test) begin
		...
		if (test)
			m_spriteBmp[spriteno] <= 52'h05555AAAAFFFF;
	end
default:	;
endcase
...
begin
  for (n = 0; n < NSPR; n = n + 1)
    if (spriteActive[n] & spriteShift[n]) begin
      sprite_on[n] <=
        spriteLink1[n] ? |{spriteBmp[(n+1)&(NSPR-1)][51:50],spriteBmp[n][51:50]} : 
        |spriteBmp[n][51:50];
    end
    else
      sprite_on[n] <= 1'b0;
...
`ifdef SUPPORT_LOWRES    
    	case(lowres)
    	2'd0,2'd3:	spriteBmp[n] <= {spriteBmp[n][49:0],2'h0};
    	2'd1:	if (hctr[0]) spriteBmp[n] <= {spriteBmp[n][49:0],2'h0};
    	2'd2:	if (&hctr[1:0]) spriteBmp[n] <= {spriteBmp[n][49:0],2'h0};
			endcase
`else
			spriteBmp[n] <= {spriteBmp[n][49:0],2'h0};
`endif
end
...
for (n = 0; n < NSPR; n = n + 1)
if (spriteLink1[n])
  spriteColorNdx[n] <= {n[3:0],spriteBmp[(n+1)&(NSPR-1)][51:50],spriteBmp[n][51:50]};
else if (spriteLink1[(n-1)&(NSPR-1)])
	spriteColorNdx[n] <= 8'h00;	// transparent
else
  spriteColorNdx[n] <= {n[5:0],spriteBmp[n][51:50]};
...
wire [14:0] alphaRed = (zrgb_i[20:14] * spriteColorOut2[28:21]) + (spriteColorOut2[20:14] * (9'h100 - spriteColorOut2[28:21]));
wire [14:0] alphaGreen = (zrgb_i[13:7] * spriteColorOut2[28:21]) + (spriteColorOut2[13:7]  * (9'h100 - spriteColorOut2[28:21]));
wire [14:0] alphaBlue = (zrgb_i[6:0] * spriteColorOut2[28:21]) + (spriteColorOut2[6:0]  * (9'h100 - spriteColorOut2[28:21]));
reg [20:0] alphaOut;

always @(posedge vclk)
    alphaOut <= {alphaRed[14:8],alphaGreen[14:8],alphaBlue[14:8]};
always @(posedge vclk)
    sprite_z3 <= sprite_z2;
always @(posedge vclk)
...
reg [20:0] flashOut;
wire [20:0] reverseVideoOut = spriteColorOut2[34] ? alphaOut ^ 21'h1FFFFF : alphaOut;
...
always @(posedge vclk)
    flashOut <= spriteColorOut3[33] ? (((flashcnt[5:2] & spriteColorOut3[32:29])!=4'b0000) ? reverseVideoOut : zrgb_i3) : reverseVideoOut;
always @(posedge vclk)
    zrgb_i4 <= zrgb_i3;
always @(posedge vclk)
    sprite_z4 <= sprite_z3;
