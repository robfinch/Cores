...
void Function::SaveGPRegisterVars()
{
	...
	if (rmask->NumMember()) {
		...
		for (nn = rmask->lastMember(); nn >= 0; nn = rmask->prevMember()) {
			GenerateDiadic(op_st, 0, makereg(nregs - 1 - nn), MakeIndexed(cnt, regSP));
			cnt += sizeOfWord;
		}
	}
}
		...
		if (!cpu.SupportsPush) {
			...
			for (count = nn = 0; nn < ta->length; nn++) {
				if (ta->preg[nn]) {
					...
					default:	GenerateDiadic(op_st, 0, makereg(ta->preg[nn] & 0x7fff), MakeIndexed(count*sizeOfWord, regSP)); count += 1; break;
					}
				}
			}
		}
	...
	if (ta) {
		...
		for (count = nn = 0; nn < ta->length; nn++) {
			if (ta->preg[nn]) {
				...
				default:	GenerateDiadic(op_ld, 0, makereg(ta->preg[nn] & 0x7fff), MakeIndexed(count*sizeOfWord, regSP)); count += 1; break;
				}
			}
		}
	}
...
int Function::RestoreGPRegisterVars()
{
	...
	if (save_mask->NumMember()) {
		...
		for (nn = save_mask->nextMember(); nn >= 0; nn = save_mask->nextMember()) {
			GenerateDiadic(op_ld, 0, makereg(nn), MakeIndexed(cnt, regSP));
			cnt += sizeOfWord;
		}
	}
	return (cnt2);
...
void Function::UnlinkStack()
{
	...
	GenerateDiadic(op_ld, 0, makereg(regFP), MakeIndirect(regSP));
	if (exceptions) {
		if (DoesThrow)
			GenerateDiadic(op_ld, 0, makereg(regXLR), MakeIndexed(2 * sizeOfWord, regSP));
	}
	if (!IsLeaf) {
		if (exceptions && DoesThrow) {
			GenerateDiadic(op_ld, 0, makereg(regAsm), MakeDoubleIndexed(regXoffs, regSP, 1));
			GenerateDiadic(op_mov, 0, makelreg(1), makereg(23));
		}
		else {
			GenerateDiadic(op_ld, 0, makereg(regAsm), MakeIndexed(3 * sizeOfWord, regSP));
			GenerateDiadic(op_mov, 0, makelreg(1), makereg(23));
		}
	}
...
void Function::SetupReturnBlock()
{
	...
	GenerateDiadic(op_st, 0, makereg(regFP), MakeIndirect(regSP));
	GenerateDiadic(op_st, 0, makereg(regZero), MakeIndexed(sizeOfWord, regSP));
	//	GenerateTriadic(op_stdp, 0, makereg(regFP), makereg(regZero), MakeIndirect(regSP));
	n = 0;
	if (exceptions) {
		if (DoesThrow) {
			n = 1;
			GenerateDiadic(op_st, 0, makereg(regXLR), MakeIndexed(2 * sizeOfWord, regSP));
		}
	}
	if (!IsLeaf) {
		n |= 2;
		GenerateDiadic(op_mov, 0, makereg(regAsm), makelreg(1));
		GenerateDiadic(op_st, 0, makereg(regAsm), MakeIndexed(3 * sizeOfWord, regSP));
	}
	if (exceptions) {
		if (DoesThrow)
...
void Function::GenReturn(Statement *stmt)
{
	...
	if (stmt != NULL && stmt->exp != NULL)
	{
		...
		if (ap->mode == am_imm)
			...
			if (sym->tp->GetBtp() && (sym->tp->GetBtp()->type == bt_struct || sym->tp->GetBtp()->type == bt_union || sym->tp->GetBtp()->type == bt_class)) {
				...
				if (p) {
					if (p->IsRegister)
						...
						GenerateDiadic(op_ldh, 0, makereg(1), MakeIndexed(p->value.i, regFP));
					ap2 = GetTempRegister();
					...
					if (cpu.SupportsPush) {
						...
						GenerateDiadic(op_sth, 0, makereg(1), MakeIndirect(regSP));
						GenerateDiadic(op_sth, 0, ap, MakeIndexed(sizeOfWord, regSP));
						GenerateDiadic(op_sth, 0, ap2, MakeIndexed(sizeOfWord * 2, regSP));
					}
					ReleaseTempReg(ap2);
			...
			if (isFloat)
				GenerateDiadic(op_ldf, 'd', makereg(1), ap);
			else
	...
	GenerateTriadic(op_add, 0, makereg(regSP), makereg(regSP), MakeImmediate(toAdd));
	if (!IsInline)
		GenerateMonadic(op_ret, 0, makelreg(1));
}
...
void Function::BuildParameterList(int *num, int *numa)
{
	...
	*numa = 0;
	...
	for (i = 0; i < np && i < MAX_PARMS; ++i) {
		...
		if (roundWord(sp1->tp->size) > 10 && !sp1->tp->IsVectorType())
			IsLeaf = FALSE;
