...
{ "ldf", op_ldf, 4, 1, true, am_fpreg, am_mem, 0, 0 },
{ "ldfd", op_ldfd,4,1,true, am_fpreg, am_mem,0,0 },
...
{ "movs", op_movs },
{ "movsx",op_movsx,1,1,false,am_reg,am_reg,0,0 },
{ "movzx",op_movzx,1,1,false,am_reg,am_reg,0,0 },
{ "mtfp", op_mtfp },
...
{ "st", op_st,4,0,true,am_reg,am_mem,0,0 },
{ "stb",op_stb,4,0,true,am_reg,am_mem,0,0 },
{ "stdcr", op_stdc,4,0,true, am_reg, am_mem,0,0 },
{ "stf", op_stf, 4, 0, true, am_fpreg, am_mem, 0, 0 },
{ "stfd", op_stfd,4,0,true, am_fpreg, am_mem,0,0 },
...
void GenerateByte(int64_t val)
{
	if( gentype == bytegen && outcol < 60) {
        ...
        nl();
        ofs.printf("\tdcb\t%d",(int)val & 0x1fff);
        gentype = bytegen;
        outcol = 19;
    }
	genst_cumulative += 1;
...
void GenerateChar(int64_t val)
{
	if( gentype == chargen && outcol < 60) {
        ofs.printf(",%d",(int)val & 0x1fff);
        outcol += 6;
    }
    else {
        nl();
        ofs.printf("\tdcw\t%d",(int)val & 0x1fff);
        gentype = chargen;
        outcol = 21;
    }
	genst_cumulative += 2;
