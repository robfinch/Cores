...
static void emit_insn(int64_t oc, int can_compress, int sz)
{
	...
	if (pass > 3) {
		emitCode(oc & 255LL);
		emitCode((oc >> 8LL) & 0x1FLL);
		num_bytes += 1;
		if (sz > 1) {
			emitCode((oc >> 13LL) & 255LL);
			emitCode((oc >> 21LL) & 0x1FLL);
			num_bytes += 1;
		}
		if (sz > 2) {
			emitCode((oc >> 26LL) & 255LL);
			emitCode((oc >> 34LL) & 0x1FLL);
			num_bytes += 1;
		}
		if (sz > 3) {
			emitCode((oc >> 39LL) & 255LL);
			emitCode((oc >> 47LL) & 0x1FLL);
			num_bytes += 1;
		}
		num_insns += 1;
		insnStats.total++;
