...
wire fprem = {opcode_i,func6b_i} == {`FLT2,`FREM} || {opcode_i,func6b_i} == {`FLT2S,`FREM};
wire [7:0] opcode = opcode_i;
wire [4:0] func6b = fprem ? func6b_r : func6b_i;
...
wire fstat 	= {opcode,func6b} == {`FLT1,`FSTAT};	// get status
wire fdiv	= {opcode,func6b} == {`FLT2,`FDIV};
wire ftx   	= {opcode,func6b} == {`FLT1,`FTX};		// trigger exception
wire fcx   	= {opcode,func6b} == {`FLT1,`FCX};		// clear exception
wire fex   	= {opcode,func6b} == {`FLT1,`FEX};		// enable exception
wire fdx   	= {opcode,func6b} == {`FLT1,`FDX};		// disable exception
wire fcmp	= {opcode,func6b} == {`FLT2,`FCMP};
wire frm	= {opcode,func6b} == {`FLT1,`FRM};  // set rounding mode

wire zl_op =  (opcode==`FLT1 && (
                    (func6b==`FABS || func6b==`FNABS || func6b==`FMOV || func6b==`FNEG || func6b==`FSIGN || func6b==`FMAN)) ||
                    func6b==`FCMP);
wire loo_op = (opcode==`FLT1 && (func6b==`ITOF || func6b==`FTOI)) || ((opcode==`FLT2 || opcode==`FLT2S) && (func6b==`TRUNC || func6b==`NXTAFT));
wire loo_done;
...
fpLOOUnit #(FPWID) u7 (.clk(clk), .ce(pipe_ce), .opcode(ir[7:0]), .func5(func6b), .rm(insn_rm==3'b111 ? rm : insn_rm), .a(aop), .b(bop), .o(looq_o), .done() );
...
reg under,unders;
wire sqrneg;
wire fms = opcode==`FMS || opcode==`FNMS;
wire nma = opcode==`FNMA || opcode==`FNMS;
...
begin
  ...
  if (ld|rem_ld)
    ...
    endcase
  	...
  	if ((opcode==`FLT2||opcode==`FLT2S) && func6b==`FDIV && divDone)
  		fpcnt <= 8'h12;
  	else
    	fpcnt <= fpcnt - 1;
   end
