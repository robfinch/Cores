...
private: void UpdateForm() {
				 ...
				 if (lastSP != cpu1.regs[30]) {
				     ...
				     for (nn = -128; nn < 128; nn+=8) {
						 ...
						 sprintf(buf, "%s %06X: %016I64X", kk == cpu1.regs[30] ? "SP->" : "    ", (unsigned int)kk,
				 ...
				 jj = 0;
				 if (lastBP != cpu1.regs[27]) {
					 sprintf(buf, "%06X", cpu1.regs[27]);
					 this->textBoxBP->Text = gcnew String(buf);
					 this->listBoxBP->Items->Clear();
					 for (nn = -128; nn < 128; nn+=8) {
						 kk = max(0,cpu1.regs[27]+nn);
						 sprintf(buf, "%s %06X: %016I64X", kk == cpu1.regs[27] ? "BP->" : "    ", (unsigned int)kk,
							 ((unsigned __int64)system1.Read(kk+4) << 32)|(unsigned __int64)system1.Read(kk));
