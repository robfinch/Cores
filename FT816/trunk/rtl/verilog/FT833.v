...
module FT833(corenum, rst, clk, clko, cyc, phi11, phi12, phi81, phi82, nmi, irq1, irq2, irq3, abort, e, mx, rdy, be, vpa, vda, mlb, vpb,
...
reg mib;
reg ssm;
reg os;                     // offset switched flag
reg [31:0] offset, offsetl;
reg [255:0] offset_stack;
...
wire [7:0] srx = {os,iml,ssm,mib,m832,m816};
reg nmi1,nmi_edge;
reg wai;
reg [31:0] a32,b32;
...
`ifdef SUPPORT_NEW_INSN
		...
		`LDO_IMM: begin offsetl <= res32; os <= `TRUE; end
		`PLO:	begin offsetl <= offset_stack[31:0]; offset_stack <= {32'd0,offset_stack[255:32]}; os <= `TRUE; end
		`PHO:   begin offset_stack <= {offset_stack[223:0],offset}; end
		`TAO:   begin offsetl <= res32; os <= `TRUE; end
		`TOA:   begin acc <= res32; nf <= resn32; zf <= resz32; os <= `FALSE; end
`endif
...
`endif
		...
		`RTI:	begin
		        ...
		        its <= ima[3];
		        ima <= {4'b0,ima[15:4]};
		        offset <= offset_stack[31:0];
		        offset_stack <= {32'd0,offset_stack[255:32]};
                inc_sp();
                if (bf) pc_cap <= TRUE;
                load_what <= `SR_70;
                state <= LOAD_MAC1;
...
`endif
        ...
        `BRK,`BRK2:
            begin
                ...
                offset_stack <= {offset_stack[223:0],offset};
                offset <= 32'd0;
                radr <= vect;
                load_what <= `LDW_TR70;
                next_state(LOAD_MAC1);
