...
//	ReleaseTempRegister(ap);
	if (cpu.SupportsBitfield) {
		if (isSigned)
			Generate4adic(op_ext,0,ap3, ap3, MakeImmediate((int64_t) node->bit_offset), MakeImmediate((int64_t)(node->bit_width-1)));
		else
			Generate4adic(op_extu,0,ap3, ap3, MakeImmediate((int64_t) node->bit_offset), MakeImmediate((int64_t)(node->bit_width-1)));
	}
	else {
		if (node->bit_offset > 0)
			GenerateTriadic(op_lsr, 0, ap3, ap3, MakeImmediate((int64_t) node->bit_offset));
...
void CodeGenerator::GenerateBitfieldInsert(Operand *ap1, Operand *ap2, int offset, int width)
{
	...
	*/
	{
		if (cpu.SupportsBitfield) {
			ap1->MakeLegal(am_reg, 8);
			ap2->MakeLegal(am_reg, 8);
			Generate4adic(op_dep, 0, ap1, ap2, MakeImmediate(offset), MakeImmediate(width - 1));
			return;
			/*
			if (ap2->offset->i == 0) {
