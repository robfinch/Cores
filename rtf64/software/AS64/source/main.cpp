...
{
	emitByte(cd & 0xFFLL);
	emitByte((cd >> 8LL) & 0xFFLL);
}
void emitTetra(int64_t cd)
{
	emitWyde(cd & 0xFFFFLL);
	emitWyde((cd >> 16LL) & 0xFFFFLL);
}
void emitOcta(int64_t cd)
{
	emitTetra(cd & 0xFFFFFFFFLL);
	emitTetra((cd >> 32LL) & 0xFFFFFFFFLL);
}
