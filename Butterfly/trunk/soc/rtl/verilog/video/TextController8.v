...
module TextController8(rst_i, clk_i, cs_regs_i, cs_ram_i, cyc_i, stb_i, ack_o, we_i, adr_i, dat_i, dat_o, vclk, hsync, vsync, blank, border, rgbIn, rgbOut);
...
// I/O range Dx
//--------------------------------------------------------------------
wire cs_text = cyc_i & stb_i & cs_ram_i;
wire cs_rom  = 1'b0;
wire cs_reg  = cyc_i & stb_i & cs_regs_i;
wire cs_any = cs_text|cs_rom|cs_reg;

// Register outputs
always @(posedge clk)
	if (cs_reg) dat_o <= rego;
	else dat_o <= adr_i[0] ? tdat_o[15:8] : tdat_o[7:0];
...
assign ack_o = cs_any ? (we_i ? 1'b1 : rdy2) : 1'b0;
...
*/
		...
		if (cs_reg & ~rw) begin	// register write ?
			...
			case(adr_i[3:0])
			4'd00:	begin
					numCols    <= dat_i;		// horizontal displayed
					//charOutDelay <= dat_i[31:16];
					end
			4'd01:	numRows    <= dat_i;
			4'd02:	windowLeft[7:0] <= dat_i;
			4'd03:  windowLeft[11:8] <= dat_i[3:0];
			4'd04:	windowTop[7:0] <= dat_i;		// vertical sync position
			4'd05:  windowTop[8] <= dat_i[0];
			4'd06:	maxScanline <= dat_i[4:0];
			4'd07:	begin
					pixelHeight <= dat_i[7:4];
					pixelWidth  <= dat_i[3:0];	// horizontal pixel width
					end
			4'd08:	txtTcCode   <= dat_i[4:0];
			4'd09:	begin
					cursorStart <= dat_i[4:0];	// scan line sursor starts on
					rBlink      <= dat_i[7:5];
					//cursorType  <= dat_i[9:8];
					end
			4'd10:	cursorEnd   <= dat_i[4:0];	// scan line cursor ends on
			4'd11:	startAddress[7:0] <= dat_i;
			4'd12:	startAddress[15:8] <= dat_i;
			4'd13:	cursorPos[7:0] <= dat_i;
			4'd14:	cursorPos[15:8] <= dat_i;
			endcase
		end
	end
