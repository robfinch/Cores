...
void gen_b(ENODE *node, int op, int label, int predreg)
{
	int size;
    ...
    GeneratePredicatedMonadic(predreg,PredOp(op),op_bra,0,make_label(label));
	ReleaseTempRegister(ap2);
	ReleaseTempRegister(ap1);
}
/*
...
{       AMODE  *ap1,*ap2;
        ...
        switch( node->nodetype )
                {
                case en_eq:	gen_b(node, op_eq, label, predreg); break;
                case en_ne: gen_b(node, op_ne, label, predreg); break;
                case en_lt: gen_b(node, op_lt, label, predreg); break;
                case en_le:	gen_b(node, op_le, label, predreg); break;
                case en_gt: gen_b(node, op_gt, label, predreg); break;
                case en_ge: gen_b(node, op_ge, label, predreg); break;
                case en_ult: gen_b(node, op_ltu, label, predreg); break;
                case en_ule: gen_b(node, op_leu, label, predreg); break;
                case en_ugt: gen_b(node, op_gtu, label, predreg); break;
                case en_uge: gen_b(node, op_geu, label, predreg); break;
                case en_land:
                        lab0 = nextlabel++;
...
//                        GenerateDiadic(op_tst,siz1,ap1,0);
                        ...
                        GeneratePredicatedMonadic(predreg,PredOp(op_ne),op_bra,0,make_label(label));
                        break;
                }
}
...
void GenerateFalseJump(ENODE *node,int label, int predreg)
{
	AMODE *ap,*ap1,*ap2;
        ...
        switch( node->nodetype )
                {
                case en_eq:	gen_b(node, op_ne, label, predreg); break;
                case en_ne: gen_b(node, op_eq, label, predreg); break;
                case en_lt: gen_b(node, op_ge, label, predreg); break;
                case en_le: gen_b(node, op_gt, label, predreg); break;
                case en_gt: gen_b(node, op_le, label, predreg); break;
                case en_ge: gen_b(node, op_lt, label, predreg); break;
                case en_ult: gen_b(node, op_geu, label, predreg); break;
                case en_ule: gen_b(node, op_gtu, label, predreg); break;
                case en_ugt: gen_b(node, op_leu, label, predreg); break;
                case en_uge: gen_b(node, op_ltu, label, predreg); break;
                case en_land:
                        GenerateFalseJump(node->p[0],label, predreg);
...
//                        GenerateDiadic(op_tst,siz1,ap,0);
                        ...
                        GeneratePredicatedMonadic(predreg,PredOp(op_eq),op_bra,0,make_label(label));
                        break;
                }
}
