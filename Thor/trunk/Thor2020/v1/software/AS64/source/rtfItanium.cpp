...
#define RII(x)	((((x) & 0x1ffffLL) << 16LL) | ((((x) >> 17LL) & 0x1fLL) << 35LL))
#define MLI(x)	((((x) & 0x1ffffLL) << 16LL) | ((((x) >> 17LL) & 0x1fLL) << 35LL))
#define MSI(x)	(((x) & 0x3fLL) | ((((x) >> 6LL) & 0x7ffLL) << 22LL) | ((((x) >> 17LL) & 0x1fLL) << 35LL))
...
{
	printf("%s. %s.%d\n", msg, /*mname.c_str(), */currentFilename, lineno);
}
...
//	case tk_end: goto j1;
	...
	case tk_file:
		...
		if (token == tk_strconst) {
			...
			strncpy(currentFilename, mname.c_str(), 299);
		}
		NextToken();
		if (token == ',') {
			NextToken();
