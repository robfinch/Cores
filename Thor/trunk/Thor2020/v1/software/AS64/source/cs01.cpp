...
#define OP6(x)	(((x) & 63LL) << 26LL)
#define FN6(x)	(((x) & 0x3fLL))
#define RT(x)		(((x) & 31LL) << 21)
#define RA(x)		(((x) & 31LL) << 16LL)
#define RB(x)		(((x) & 31LL) << 11LL)
#define RC(x)		(((x) & 31LL) << 6LL)
#define RII(x)	((x) & 0xffffLL)
#define MLI(x)	((x) & 0xffffLL)
#define MSI(x)	(((x) & 0x7ffLL) | ((((x) >> 11LL) & 0x1fLL) << 21LL))
#define RM(x)		(((x) & 7LL) << 6LL)
...
//		getSz(&sz);
  ...
  val = expr128();
	...
	if (!IsNBit128(val, *Int128::MakeInt128(16LL))) {
...
static void process_rrop()
{
  ...
  Rb = getRegisterX();
	...
	//	goto xit;
	//}
  emit_insn(instr | FN6(funct6)|RB(Rb)|RT(Rt)|RA(Ra));
	xit:
		prevToken();
		ScanToEOL();
}
