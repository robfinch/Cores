...
extern void Thor2020_processMaster();
...
extern void FT64v8_processMaster();
...
void displayHelp()
{
     ...
     printf("    +g[n]   = cpu version 8=Table888, 9=Table888mmu V=RISCV 6=FISA64 T=Thor 2=Thor2020\r\n");
     printf("                  I=Itanium  D=DSD6 7=DSD7 A=DSD9 F=FT64 G=FT64x36 X=FT8088 N=NVIO3\r\n");
		 printf("                  O=Gambit  O5=Gambit_v5\r\n");
...
void emitPentabyte(Int128 cd)
{
	emitByte(cd.low & 0xffLL);
	emitByte((cd.low >> 8LL) & 0xFFLL);
	emitByte((cd.low >> 16LL) & 0xFFLL);
	emitByte((cd.low >> 24LL) & 0xFFLL);
	emitByte((cd.low >> 32LL) & 0xFFLL);
}
        ...
        if (segment==dataseg) {
            ...
            if (first_org && segment==codeseg) {
              ...
              first_org = 0;
							...
							if (!isInitializationData) {
								if (gCpu == THOR2020) {
									//if (sections[0].address < new_address*mul)
									//	fprintf(ofp, "%06I64X ", sections[0].address);
									cnt = 0;
									ndx = sections[0].index;
								}
								while (sections[0].address < new_address*mul) {
									...
									if (gCpu == THOR2020) {
										if (cnt == 16) {
											...
											fprintf(ofp, "%06I64X ", sections[0].address);
											for (nn = 0; nn < 16; nn++)
												fprintf(ofp, "%02X ", sections[0].bytes[ndx+nn]);
											fprintf(ofp, "\n");
										}
									}
								}
		...
		if (*inptr == '"') {
			...
			while (*inptr != '"') {
				if (*inptr == '\\') {
					inptr++;
					switch (*inptr) {
					case '\\': emitHalf('\\'); emitByte(0); inptr++; break;
					case 'r': emitHalf(0x0D); emitByte(0); inptr++; break;
					case 'n': emitHalf(0x0A); emitByte(0); inptr++; break;
					case 'b': emitHalf('\b'); emitByte(0); inptr++; break;
					case '"': emitHalf('"'); emitByte(0); inptr++; break;
					default: inptr++; break;
					}
				}
...
void processFile(char *fname, int searchincl)
{
  ...
  char *pathname;
	...
	strncpy(currentFilename, fname, 299);
  pathname = (char *)NULL;
	ifs.open(fname);
  if (ifs.fail()) {
    if (searchincl) {
...
void processMaster()
{
	...
	case THOR2020: Thor2020_processMaster(); break;
    ...
    if (verilog_out) {
        ...
        if (vfp) {
			...
			*/
			...
			else if (gCpu == 'F' || gCpu == 'G' || gCpu == 'J' || gCpu==THOR2020) {
				if (vebits == 256) {
...
bool IsNBit128(Int128& val, Int128& n)
{
	Int128 low, high;
	Int128 d, s;
	Int128::Sub(&d, &n, Int128::One());	// n - 1
	int m;

	Int128::Assign(&s, Int128::One());
	for (m = 0; m < d.low; m++)
		Int128::Shl(&s, &s);
	Int128::Assign(&high, &s);
	Int128::Sub(&low, Int128::Zero(), &s);
	return (Int128::IsGE(&val, &low) && Int128::IsLessThan(&val, &high));
}
