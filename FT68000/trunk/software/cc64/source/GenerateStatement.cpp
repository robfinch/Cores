...
{    
    ...
    ap = GenerateExpression(exp,F_DREG,GetNaturalSize(exp));
    for(stmt = s1; stmt != NULL; stmt = stmt->next )
    {
        ...
        else
        {
			...
			for (nn = (int)bf[0]; nn >= 1; nn--) {
				GenerateDiadic(op_cmp,'l',make_immed(bf[nn]),ap);
				GenerateMonadic(op_beq,0,make_clabel(curlab));
			}
            stmt->label = (int64_t *)curlab;
        }
...
j1:	;
	    ...
	    ap = GenerateExpression(exp,F_DREG,GetNaturalSize(exp));
		if (!nkd) {
			GenerateDiadic(op_cmp,'l',make_immed(minv),ap);
			GenerateMonadic(op_blt,0,make_clabel(defcase ? (int)defcase->label : breaklab));
			GenerateDiadic(op_cmp,'l',make_immed(maxv+1),ap);
			GenerateMonadic(op_bge,0,make_clabel(defcase ? (int)defcase->label : breaklab));
			//Generate4adic(op_chk,0,ap,ap1,ap2,make_clabel(defcase ? (int)defcase->label : breaklab));
		}
		if (minv != 0)
			GenerateDiadic(op_sub,'l',make_immed(minv),ap);
		GenerateDiadic(op_asl,'l',make_immed(2),ap);
		GenerateDiadic(op_move,'l',make_indexed2(tablabel,ap->preg),ap);
		GenerateMonadic(op_jmp,0,make_indexed(0,ap->preg));
		s1->GenerateCase();
		GenerateLabel(breaklab);
		return;
	}
...
void Statement::GenerateTry()
{
	...
	a->mode = am_immed;
	GenerateDiadic(op_move,'l',a,make_areg(regXLR));
	s1->Generate();
	...
	for (stmt = s2; stmt; stmt = stmt->next) {
		...
		if (stmt->num==99999)
			...
			ap2 = GetTempRegister();
			GenerateDiadic(op_move,'l',make_immed(stmt->num),ap2);
			ReleaseTempReg(ap2);
			GenerateTriadic(op_bne,0,make_dreg(1),ap2,make_clabel(nextlabel));
		}
		// move the throw expression result in 'r1' into the catch variable.
		node = stmt->exp;
		ap2 = GenerateExpression(node,F_AREG|F_DREG|F_MEM,GetNaturalSize(node));
		if (ap2->mode==am_dreg || ap2->mode==am_areg)
			GenerateDiadic(op_move,'l',make_dreg(0),ap2);
		else
			GenStore(make_dreg(0),ap2,GetNaturalSize(node));
	...
	a->mode = am_immed;
	GenerateDiadic(op_move,'l',make_areg(regXLR),a);
}
...
void Statement::GenerateThrow()
{
    ...
    if(exp != NULL )
	{
		...
		else if( ap->mode != am_dreg && ap->mode != am_areg)
			GenerateDiadic(op_move,'l',ap,make_dreg(0));
		else if (ap->preg != 1 )
			GenerateDiadic(op_move,'l',ap,make_dreg(0));
		ReleaseTempRegister(ap);
		GenerateDiadic(op_move,'l',make_immed(num),make_dreg(1));
	}
...
void Statement::GenerateCheck()
{
    ...
    */
	...
	ap2 = GenerateExpression(node->p[1],F_DREG,size);
    ap3 = GenerateExpression(node->p[2],F_DREG|F_IMMED,size);
...
void Statement::GenerateFirstcall()
{
    ...
    if( s1 != NULL )
    {
        breaklab = nextlabel++;
		ap1 = GetTempRegister();
		GenerateMonadic(op_tst,'l',make_string(fcname));
       	GenerateMonadic(op_beq,0,make_clabel(breaklab));
		ReleaseTempRegister(ap1);
		GenerateMonadic(op_clr,'l',make_string(fcname));
		s1->Generate();
        GenerateLabel(breaklab);
        breaklab = lab2;
    }
