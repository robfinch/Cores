...
`define TRAP_TRAP       9'h020
`define IRQ_TRAP        9'h1F0
`define TRAPV_TRAP      9'h1F8
`define ILLEGAL_TRAP    9'h1F9
`define NMI_TRAP        9'h1FE
`define RESET_TASK      9'h1FF
...
// MOVE.W
//-----------------------------------------------------------------------------
	4'h2:	fs_data(mmm,rrr,FETCH_WORD,STORE_IN_DEST,S);
//-----------------------------------------------------------------------------
// MOVE.L
//-----------------------------------------------------------------------------
	4'h3:	fs_data(mmm,rrr,FETCH_LWORD,STORE_IN_DEST,S);
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
	4'h4:
		casex(ir[11:8])
...
CMP1:
	begin
		state <= IFETCH;
		case(sz)
		2'b00:	begin zf <= resB== 8'd0; nf <= resB[ 7]; cf <= resB[ 8]; vf <= resB[ 8]!=resB[ 7]; end
		2'b01:	begin zf <= resW==16'd0; nf <= resW[15]; cf <= resW[16]; vf <= resW[16]!=resW[15]; end
		2'b10:	begin zf <= resL==32'd0; nf <= resL[31]; cf <= resL[32]; vf <= resL[32]!=resL[31]; end
		2'b11:
			if (ir[8])
				begin zf <= resL==32'd0; nf <= resL[31]; cf <= resL[32]; vf <= resL[32]!=resL[31]; end
			else
				begin zf <= resW==16'd0; nf <= resW[15]; cf <= resW[16]; vf <= resW[16]!=resW[15]; end
		endcase
	end
...
SHIFT:
	if (cnt!=5'd0) begin
		...
		Rt <= {1'b0,rrr};
		case(sz)
		2'b00:	begin zf <= resB== 8'h00; nf <= resB[ 7]; end
		2'b01:	begin zf <= resW==16'h00; nf <= resW[15]; end
		2'b10:	begin zf <= resL==32'h00; nf <= resL[31]; end
		2'b11:	begin zf <= resW==16'h00; nf <= resW[15]; end
		endcase
		case(sz)
...
TRAP:
    begin
        ...
        if (is_nmi)
            trapno <= `NMI_TRAP;
        else if (is_irq) begin
            trapno <= `IRQ_TRAP + ipl_i;
            im <= ipl_i;
        end
        else begin
            if (ir[3:0]==4'hF)
...
TRAP3:
    if (!cyc_o) begin
        cyc_o <= `HIGH;
        stb_o <= `HIGH;
        sel_o <= trapno[0] ? 4'b1100 : 4'b0011;
        adr_o <= {trapno,1'b0};
    end
    else if (ack_i) begin
        otr <= tr;
        tr <= trapno[0] ? dat_i[24:16] : dat_i[8:0];
        state <= TASK2;        
    end
