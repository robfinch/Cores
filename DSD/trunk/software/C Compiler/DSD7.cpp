...
*/	
			if (regno) {
				...
				if (ap->mode==am_immed)
					GenerateDiadic(op_ld,0,makereg(regno), ap);
				else
...
*/
	...
	if (i!=0) {
		if (sym) {
			if (!sym->IsPascal)
				GenerateTriadic(op_add,0,makereg(regSP),makereg(regSP),make_immed(i * 2));
		}
		else
			GenerateTriadic(op_add,0,makereg(regSP),makereg(regSP),make_immed(i * 2));
	}
