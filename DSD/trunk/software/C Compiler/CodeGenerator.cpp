...
void GenLoad(AMODE *ap3, AMODE *ap1, int ssize, int size)
{
    if (ap3->isFloat) {
        ...
        if (ap3->isUnsigned) {
        	switch(size) {
        	case 1:	GenerateDiadic(op_lhu,0,ap3,ap1); break;
...
void GenStore(AMODE *ap1, AMODE *ap3, int size)
{
    if (ap1->isFloat) {
    	...
    	switch(size) {
    	case 1: GenerateDiadic(op_sh,0,ap1,ap3); break;
    	case 2: GenerateDiadic(op_sw,0,ap1,ap3); break;
    	}
    }
}
...
//	case 1:	GenerateTriadic(op_andi,0,ap,ap,make_immed(0xFF)); break;
	case 1:	GenerateTriadic(op_andi,0,ap,ap,make_immed(0xFFFF)); break;
//	case 2:	GenerateTriadic(op_andi,0,ap,ap,make_immed(0xFFFFFFFF)); break;
    }
}
...
{       
    ...
    {       /* both nodes are registers */
        ...
        ap2 = GenerateExpression(node->p[1],F_REG,2);
        ap1->mode = am_indx2;
        ap1->sreg = ap2->preg;
		ap1->deep2 = ap2->deep2;
    ...
    ap1 = GenerateExpression(node->p[0],F_REG | F_IMMED,2);
    ...
    ap2 = GenerateExpression(node->p[1],F_ALL,2);   /* get right op */
...
AMODE *GenerateAssignAdd(ENODE *node,int flags, int size, int op)
{
	...
	if (ap2->mode ==am_immed)
		switch(op) {
		case op_addu:	op = op_addi; break;
		case op_add:	op = op_addi;  break;
		case op_subu:	op = op_subi; negf = true; break;
		case op_sub:	op = op_subi; negf = true; break;
		}
	if (ap1->mode==am_reg) {
...
//    case en_fptempref:
    ...
    case en_d2i:
         ap1 = GetTempRegister();	
         ap2 = GenerateExpression(node->p[0],F_FPREG,2);
    ...
    case en_land:
		...
		ReleaseTempRegister(ap2);
		ReleaseTempRegister(ap1);
		GenerateDiadic(op_tst,0,makepred(maxPn),ap3);
		GeneratePredicatedDiadic(PredOp(op_ne),maxPn,op_ldi,0,ap3,make_immed(1));
		GeneratePredicatedDiadic(PredOp(op_eq),maxPn,op_ldi,0,ap3,make_immed(0));
		return ap3;
	...
	case en_not:
		...
		size = GetNaturalSize(node);
		ap1 = GenerateExpression(node->p[0],F_REG,size);
		GenerateDiadic(op_tst,0,makepred(maxPn),ap1);
		GeneratePredicatedDiadic(PredOp(op_ne),maxPn,op_ldi,0,ap1,make_immed(0));
		GeneratePredicatedDiadic(PredOp(op_eq),maxPn,op_ldi,0,ap1,make_immed(1));
        return ap1;
    case en_chk:
        return GenExpr(node);
  ...
  //          return ap1;
		//}
			...
			ap1 = GenerateExpression(node->p[0],F_REG, size);
			ap2 = GenerateExpression(node->p[1],F_REG|F_IMMED,size);
			GenerateTriadic(ap2->mode==am_immed ? op_cmpi : op_cmp,0,makepred(maxPn),ap1,ap2);
			ReleaseTempRegister(ap2);
			ReleaseTempRegister(ap1);
			ap3 = GetTempRegister();
			GeneratePredicatedDiadic(PredOp(op_ne),maxPn,op_ldi,0,ap3,make_immed(1));
