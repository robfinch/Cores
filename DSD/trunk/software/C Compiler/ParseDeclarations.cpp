...
//printf("Enter ParseInt\r\n");
	if (isUnsigned) {
		head = TYP::Make(bt_ulong,2);
		tail = head;
	}
	else {
		head = TYP::Make(bt_long,2);
		tail = head;
  }
	if (head==nullptr)
    return;
...
//				tail = head;
			...
			case ellipsis:
				head = (TYP *)TYP::Make(bt_ellipsis,2);
				tail = head;
				head->isVolatile = isVolatile;
				head->isIO = isIO;
				head->isConst = isConst;
      ...
      case kw_exception:
				head = (TYP *)TYP::Make(bt_exception,2);
				tail = head;
				head->isVolatile = isVolatile;
				head->isIO = isIO;
				head->isConst = isConst;
