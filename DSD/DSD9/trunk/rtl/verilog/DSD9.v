...
`define SIMULATION
`define VBA_VECT    48'hFFFFFFFFFF00
`define RST_VECT    48'hFFFFFFFC0000
...
`define FLT_ALIGN   9'd487


module DSD9(hartid_i, rst_i, clk_i, irq_i, icause_i,
    cyc_o, stb_o,
    ack_i,
...
parameter LOAD2A = 6'd11;
parameter LOAD3 = 6'd12;
parameter LOAD3A = 6'd13;
parameter LOAD4 = 6'd14;
parameter INVnRUN = 6'd15;
parameter LOAD5 = 6'd16;
parameter LOAD6 = 6'd17;
parameter STORE1 = 6'd18;
parameter STORE2 = 6'd19;
parameter STORE2A = 6'd21;
parameter STORE3 = 6'd22;
parameter STORE3A = 6'd23;
parameter STORE4 = 6'd24;
parameter LOAD_ICACHE = 6'd25;
parameter LOAD_ICACHE2 = 6'd26; 
parameter LOAD_ICACHE3 = 6'd27; 
parameter ICACHE_RST = 6'd28;
...
reg [DMSB:0] pc,dpc,xpc;
reg [DMSB:0] pc_inc;
reg [119:0] insn,iinsn;
reg [119:0] ir,xir,mir;
reg [15:0] fault_insn;
reg stuff_fault;
reg ii48,ii96,ii5,ii5a,ii596,ii5a96;
reg i48,i96,i5,i596,i5a,i5a96;
wire [7:0] iopcode = iinsn[7:0];
wire [7:0] ifunct = iinsn[39:32];
wire [7:0] iRb = iinsn[23:16];
wire [7:0] opcode = ir[7:0];
wire [7:0] funct = ir[39:32];
reg [7:0] Ra,iRa,xRa;
reg [7:0] Rb,xRb;
reg [7:0] Rc;
reg [7:0] Rt,xRt;
...
reg [3:0] isp;
reg [DMSB:0] r1,r2,r3;
reg [DMSB:0] regfile [0:63];
reg [DMSB:0] sp;
reg [DMSB:0] rfoa,rfob,rfoc;
reg [DMSB:0] a,b,c,imm;
reg [DMSB:0] res,lres;
reg [DMSB:0] ea,xb,lres1,res2;
wire takb;
reg [DMSB:0] br_disp;
wire [DMSB:0] logic_o, shift_o;
reg [1:0] mem_size;
reg upd_rf;                     // flag indicates to update register file
reg xinv,dinv;                  // invalidate flags for pipeline stages
reg [1:0] dinc_amt,xinc_amt;
// CSR's
reg [DMSB:0] tick;
reg [DMSB:0] msema;
reg [DMSB:0] mcause;
reg [DMSB:0] vba;                 // vector table base address 
reg [DMSB:0] tr;
reg im, gie;
reg [128:0] itos;
reg [DMSB:0] cisc;
...
reg [DMSB:0] scratch;
reg [DMSB:0] cap = {CAP_MULDIV,CAP_ROTATES,CAP_LS_NDX};
reg [DMSB:0] pcr;
reg [DMSB:0] mconfig;
wire [7:0] regSP = mconfig[7:0];
wire [7:0] regBP = mconfig[15:8];
reg [DMSB:0] sbl, sbu;
...
// Increment the program counter by up to three words
...
4'h0:   case(amt)
        2'd0:   fnIncdPC = ipc;
        2'd1:   fnIncdPC = {ipc[31:4],4'h5};
        2'd2:   fnIncdPC = {ipc[31:4],4'hA};
        2'd3:   fnIncdPC = {ipc[31:4]+28'd1,4'h0};   
        endcase
4'h5:   case(amt)
        2'd0:   fnIncdPC = ipc;
        2'd1:   fnIncdPC = {ipc[31:4],4'hA};
        2'd2:   fnIncdPC = {ipc[31:4]+28'd1,4'h0};   
        2'd3:   fnIncdPC = {ipc[31:4]+28'd1,4'h5};   
        endcase
4'hA:   case(amt)
        2'd0:   fnIncdPC = ipc;
        2'd1:   fnIncdPC = {ipc[31:4]+28'd1,4'h0};
        2'd2:   fnIncdPC = {ipc[31:4]+28'd1,4'h5};   
        2'd3:   fnIncdPC = {ipc[31:4]+28'd1,4'hA};   
        endcase
default:    fnIncdPC = 32'h00000000;    // Set this to transfer to unaligned instruction fault
endcase
endfunction

function [DMSB:0] fnAbs;
input [DMSB:0] jj;
fnAbs = jj[DMSB] ? -jj : jj;
endfunction
...
`LD32,`LDU32,`LD96,`LDR96,`ST32,`ST96,`STC96,`PEA,
...
`R2:
    ...
    `LDX32,`LDUX32,`LDX96,`LDRX96,`STX32,`STX96,`STCX96:
    xIsMC = `TRUE;
    endcase
default:    xIsMC = `FALSE;
endcase
...
wire [(((DMSB+1)*2)-1):0] mul_prod1;
reg [(((DMSB+1)*2)-1):0] mul_prod;
reg mul_sign;
reg [DMSB:0] aa, bb;
...
begin
    ...
    `R2:
        ...
        `SHRI:  res = shift_o;
        `ASRI:  res = shift_o;
        `ROLI:  res = CAP_ROTATES ? shift_o : 32'hDEADDEAD;
        `RORI:  res = CAP_ROTATES ? shift_o : 32'hDEADDEAD;
        `SXC:   res = {{32{a[15]}},a[15:0]};
        `SXH:   res = {{24{a[23]}},a[23:0]};
        ...
        `LDX32,`LDUX32,`LDX96,`LDRX96:  res = CAP_LS_NDX ? lres : 32'hDEADDEAD;
    ...
    `MEM:
        ...
        default:    res = 32'hDEADDEAD;
        endcase
    `LD32,`LDU32,`LD96,`LDR96:  res = lres[31:0];
    `PEA:       res = a - 32'd12;
    `CALL:      res = a - 32'd12;
    default:    res = 32'hDEADDEAD;
    endcase
...
//
//---------------------------------------------------------------------------
wire [31:0] pcp8 = fnIncdPC(2'd3,pc);   // Use pc plus 3 to select the next cache line.
wire [22:0] ic_lfsr;
...
`endif
    if (isICacheReset) begin
        case(adr_o[3:2])
        2'd0: cache_mem0[adr_o[`adrN]][31:0] <= dat_i;
        2'd1: cache_mem0[adr_o[`adrN]][63:32] <= dat_i;
        2'd2: cache_mem0[adr_o[`adrN]][95:64] <= dat_i;
        2'd3: cache_mem0[adr_o[`adrN]][127:96] <= dat_i;
        endcase
        case(adr_o[2:1])
        2'd0: cache_mem1[adr_o[`adrN]][31:0] <= dat_i;
        2'd1: cache_mem1[adr_o[`adrN]][63:32] <= dat_i;
        2'd2: cache_mem1[adr_o[`adrN]][95:64] <= dat_i;
        2'd3: cache_mem1[adr_o[`adrN]][127:96] <= dat_i;
        endcase
        case(adr_o[2:1])
        2'd0: cache_mem2[adr_o[`adrN]][31:0] <= dat_i;
        2'd1: cache_mem2[adr_o[`adrN]][63:32] <= dat_i;
        2'd2: cache_mem2[adr_o[`adrN]][95:64] <= dat_i;
        2'd3: cache_mem2[adr_o[`adrN]][127:96] <= dat_i;
        endcase
        case(adr_o[2:1])
        2'd0: cache_mem3[adr_o[`adrN]][31:0] <= dat_i;
        2'd1: cache_mem3[adr_o[`adrN]][63:32] <= dat_i;
        2'd2: cache_mem3[adr_o[`adrN]][95:64] <= dat_i;
        2'd3: cache_mem3[adr_o[`adrN]][127:96] <= dat_i;
        endcase
    end
    ...
    // This currently requires 4 bus cycles of 32 bits.
        ...
        case({ic_whichWay,adr_o[3:2]})
        4'd0: cache_mem0[adr_o[`adrN]][31:0] <= dat_i;
...
`endif
    if (isICacheReset|isICacheLoad) begin
        case(adr_o[3:2])
        2'd0: cache_mem[adr_o[`adrN]][31:0] <= dat_i;
        2'd1: cache_mem[adr_o[`adrN]][63:32] <= dat_i;
        2'd2: cache_mem[adr_o[`adrN]][95:64] <= dat_i;
        2'd3: cache_mem[adr_o[`adrN]][127:96] <= dat_i;
        endcase
    end
    end
// Pull instructions from four pairs of cache lines, one for each way. Typically
// only a single pair of cache lines will contain the valid instructions.
wire [127:0] co1 = cache_mem[pc[`adrN]];
wire [127:0] co2 = cache_mem[pcp8[`adrN]];
// Get the instruction window for the pipeline. Instructions can be up to 64 bit long
// and spanning cache lines.
// Combine the cache line pair selected in case the instruction spans cache lines.
always @(pc or co1 or co2)
case(pc[3:0])
4'h0: insn = co1[119:0];
//4'h5: insn = {co2[39:0],co1[119,40]};
//4'hA: insn = {co2[79:0],co1[119,80]};
default: insn = {`FLT_ALIGN,`INT};
endcase 
...
always @(posedge clk_i)
    ...
    if (adr_o[3:2]==2'b11) begin
        if (isICacheReset|isICacheLoad)
            tag_mem[adr_o[`adrN]] <= adr_o;
    end

// Set tag comparators, there would be only four for a four-way set associative
// cache, but we need to check two cache lines in case the instruction spans a
// cache lines. Hence there are four pairs of comparators.
wire ihit1 = pc[31:12]==tag_mem[pc[`adrN]][31:12];
wire ihit2 = pcp8[31:12]==tag_mem[pcp8[`adrN]][31:12];
wire ihit = (ihit1 && pc[3:0]==4'h0) || (ihit1 && ihit2);
...
begin
    ...
    if (advanceIF) begin
        ...
        if (stuff_fault) begin
            ...
            iinsn = insn;
        ii48 = iinsn[39:32]==8'h80;
        ii96 = iinsn[39:32]==8'h81;
        ii5 = iinsn[23:16]==8'h80;
        ii596 = iinsn[23:16]==8'h81;
        ii5a = iinsn[15:8]==8'h80;
        ii5a96 = iinsn[15:8]==8'h81;
        iRa = iinsn[10:6]; 
        ...
        else if (iinsn[7:0]==`PEA || iinsn[7:0]==`CALL ||
            (iinsn[7:0]==`MEM && (iinsn[39:32]==`RET || iinsn[39:32]==`PUSH || insn[39:32]==`POP))) begin
            Ra <= regSP;
            Rb <= iinsn[15:8];
            Rc <= iinsn[23:16];
        end
...
//        else
        case(insn[7:0])
        `LD32,`LDU32,`LD96,`LDR96,`ST32,`ST96,`STC96,
        `MULI,`MULUI,`MULSUI,`MULHI,`MULUHI,`MULSUHI,
        `DIVI,`DIVUI,`DIVSUI,`REMI,`REMUI,`REMSUI,
        `ADDI,`CMPI,`CMPUI,`ANDI,`ORI,`XORI,
        `CALL,`JMP,
        `PEA:
            pc_inc = ii96 ? 32'd3 : ii48 ? 32'd2 : 32'd1;
        `Bcc:   pc_inc = 32'd1;
        `BccU:  pc_inc = 32'd1;
        `BccI:  pc_inc = ii596 ? 32'd3 : ii5 ? 32'd2 : 32'd1;
        `BccUI:  pc_inc = ii596 ? 32'd3 : ii5 ? 32'd2 : 32'd1;
        `NOP,`CINSN:
            pc_inc = 32'd1;
        `CSRI:  pc_inc = ii5a96 ? 32'd3 : ii5a ? 32'd2 : 32'd1;
        `CSR:   pc_inc = 32'd1;
        `R2:
            case(insn[31:26])
            `R2CSRI:  pc_inc = ii5a96 ? 32'd3 : ii5a ? 32'd2 : 32'd1;
            default:    pc_inc = 32'd1;
            endcase
        `MEM:
            case(insn[15:11])
            `RET,`PUSHI5:   pc_inc = ii5a96 ? 32'd3 : ii5a ? 32'd2 : 32'd1;
            default:    pc_inc = 32'd1;
            endcase
        default:    pc_inc = 32'd1;
        endcase
        pc <= fnIncdPC(pc_inc,pc);
        dinc_amt <= pc_inc;
        i48 <= ii48;
        i96 <= ii96;
        i5 <= ii5;
    ...
    if (advanceRF) begin
        ...
        `ADDI,`CMPI,`CMPUI,`ANDI,`ORI,`XORI,
        `LD32,`LDU32,`LD96,`LDR96,`ST32,`ST96,`STC96,`PEA:
            imm <= i96 ? {ir[111:40],ir[31:24]} : i48 ? {{40{ir[79]}},ir[79:40],ir[31:24]} : {{72{ir[39]}},ir[39:24]};
        `BccI,`BccUI:  imm <= i596 ? {ir[111:40],ir[23:16]} : i5 ? {{40{ir[79]}},ir[79:40],ir[23:16]} : {{88{ir[23]}},ir[23:16]};
        `CSRI:         imm <= i5a96 ? {ir[111:40],ir[15:8]} : i5a ? {{40{ir[79]}},ir[79:40],ir[15:8]} : {{88{ir[15]}},ir[15:8]};
        `R2:
            case(ir[39:32])
            `R2CSRI:    imm <= i5a96 ? {ir[111:40],ir[15:8]} : i5a ? {{40{ir[79]}},ir[79:40],ir[15:8]} : {{88{ir[15]}},ir[15:8]};
            default:    imm <= 88'h0;
            endcase
        `MEM:
            case(ir[39:32])
            `RET:       imm <= i5a96 ? {ir[111:40],ir[15:8]} : i5a ? {{40{ir[79]}},ir[79:40],ir[15:8]} : {{88{ir[15]}},ir[15:8]};
            `PUSHI5:    imm <= i5a96 ? {ir[111:40],ir[15:8]} : i5a ? {{40{ir[79]}},ir[79:40],ir[15:8]} : {{88{ir[15]}},ir[15:8]};
            default:    imm <= 88'h0;
            endcase
        default:    imm <= 88'h0;
        endcase
        ...
        `MEM:
            case(xir[39:32])
            `PUSHI5,`PUSH:  xRt <= regSP;
            `RET:   xRt <= regSP;
            `POP:   begin xRt <= ir[15:8]; xRt2 <= 1'b1; end
            default:    xRt <= 5'd0;
            endcase
        `MOV,
        `ADDI,`CMPI,`CMPUI,`ANDI,`ORI,`XORI,`ORI32,
        `LD32,`LDU32,`LD96,`LDR96:
            xRt <= ir[23:16];
        `PEA:
            xRt <= regSP;
        `JMP:    xRt <= ir[23:16];
...
//        inv_xir();
    ...
    if (!xinv) begin   // if (advanceEX) // always true
        ...
        `INT:
            begin
                mcause <= xir[16:8];
                itos <= {im,c,b,a,xpc+xir[17]};
                im <= `TRUE;
                msema[0] <= 1'b0;
                ex_branch({vba[31:6],ol,6'b00});
            end
        
        `JMP:
                if (xRa!=5'd0) begin
                    if (xRa==regSP)
                       ex_branch({xpc[31:8] + imm[31:8],imm[7:0]});
                    else
                        ex_branch({a[31:8] + imm[31:8],imm[7:0]|a[7:0]});
                end
        `CALL:      ex_call(xinc_amt);

        `Bcc,`BccU,
        `BccI,`BccUI:
            if (takb)
                ex_branch({xpc[31:8] + br_disp[31:8],br_disp[7:0]});
        ...
        `R2:
            ...
            `R2CSR:   if (xRa != 8'd0)
                            write_csr(xir[26:25],b[13:0],a);
            `R2CSRI:  write_csr(xir[26:25],b[13:0],imm);
            endcase
        `SYS:
            case(xir[39:32])
            `CLI:   im <= 1'b0;
...
LOAD2A:
    begin
        read1(mem_size,ea+32'd4);
        next_state(LOAD3);
    end

// The operation here must be a LD96 or LDR96.
LOAD3:
    if (err_i) begin
        cyc_o <= `FALSE;
        stb_o <= `FALSE;
        ...
        `CALL:
            if (xRb!=8'd0) begin
                if (xRb==regSP)
                    ...
                    ex_branch({b[31:8] + imm[31:8],b[7:0]|imm[7:0]});
            end
        `MEM:
            if (xir[15:11]==`RET)
                ex_branch(lres[31:0]);
...
STORE2A:
    begin
        write1(mem_size,ea+32'd4,xb[63:32]);
        next_state(STORE3);
    end
STORE3:
    if (err_i) begin
...
STORE3A:
    begin
        write1(mem_size,ea+32'd8,xb[87:64]);
        next_state(STORE4);
    end
STORE4:
    if (err_i) begin
