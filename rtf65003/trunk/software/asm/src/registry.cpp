...
{
	// Create object.
	Registry::Registry(char *rk, int hv)
	{
		hive = hv;
	...
	int Registry::write(char *vname, int vtype, void *data, int cbData )
	{
		HKEY hKey;
		int ret;
	...
	// read a key
	int Registry::read(char *vname, void *d, int n)
	{
		HKEY hKey;
		BYTE data [1024];
		DWORD cbData = 1024;
	...
	bool Registry::check(char *vname, char *v)
	{
		char d[1000];
