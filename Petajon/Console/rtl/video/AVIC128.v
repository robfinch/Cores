...
input [15:0] aud_in;
// Debugging
output reg [7:0] state = 8'd13;	// OTHERS
	...
	if (cpu_wr_reg | reg_copper)
		...
		9'b100?_????_1:
			begin
				...
				if ( sel[3]) sprite_pz[adr[8:4]] <= dat[31:28];
				if (|sel[5:4]) sprite_ph[adr[8:4]] <= dat[43:32];
				if (|sel[7:6]) sprite_pv[adr[8:4]] <= dat[59:48];
			end

		// Raster interrupt indicators: $B80 to $BF8
		9'b1011_1???_?:	rasti_en[adr[6:3]] <= dat;
...
ST_COPPER_IFETCH:
	begin
		...
		m_sel_o <= 16'hFFFF;
		m_adr_o <= copper_pc;
		call(ST_LATCH_DATA,ST_COPPER_IFETCH2);
...
if (bltCtrl[13] & ~bltDone1)
	irq_status[1] <= `TRUE;
if (hctr==12'd02 && rasti_en[vctr[9:6]][vctr[5:0]])
	irq_status[2] <= `TRUE;
...
always @(posedge vclk)
for (n = 0; n < NSPR; n = n + 1)
if (spriteLink1[n] && spriteLink1[(n+1)&31]) begin
  spriteColorNdx[n] <= {spriteBmp[(n+2)&31][127:124],spriteBmp[(n+1)&31][127:124],spriteBmp[n][127:124]};
  use_pal <= `FALSE;
end
else if (spriteLink1[n] & ~spriteLink1[(n-1)&31]) begin
  spriteColorNdx[n] <= {spriteBmp[(n+1)&31][127:124],spriteBmp[n][127:124]};
  use_pal <= `TRUE;
end
else if (spriteLink1[(n-1)&31]) begin
	spriteColorNdx[n] <= 12'h000;	// transparent
  use_pal <= `TRUE;
end
else begin
...
always @(posedge vclk)
    border2 <= border1;
always @(posedge vclk)
    spriteColorOut2 <= use_pal1 ? sprite_color[spriteClrNdx[7:0]] :
    	{8'h00,spriteClrNdx[11:8],4'd0,spriteClrNdx[7:4],4'd0,spriteClrNdx[3:0],4'd0};
always @(posedge vclk)
    sprite_z2 <= sprite_z1;
...
wire [12:0] alphaRed = (R(rgb_i) * spriteColorOut2[31:24]) + (spriteColorOut2[`RBits] * (9'h100 - spriteColorOut2[31:24]));
wire [12:0] alphaGreen = (G(rgb_i) * spriteColorOut2[31:24]) + (spriteColorOut2[`GBits]  * (9'h100 - spriteColorOut2[31:24]));
wire [12:0] alphaBlue = (B(rgb_i) * spriteColorOut2[31:24]) + (spriteColorOut2[`BBits]  * (9'h100 - spriteColorOut2[31:24]));
reg [14:0] alphaOut;
...
always @(posedge vclk)
    rgb_i3 <= rgb_i;
always @(posedge vclk)
    zb_i3 <= zorder ? {Z(rgb_i),4'h00} : rgb_i[15] ? 8'hFF : 8'h7F;
always @(posedge vclk)
    blank3 <= blank2;
always @(posedge vclk)
    border3 <= border2;
...
always @(posedge vclk)
	...
	3'b001:		rgb <= ((zb_i4 < sprite_z4) ? {R(rgb_i4),3'b0,G(rgb_i4),3'b0,B(rgb_i4),3'b0} :
											{flashOut[14:10],3'b0,flashOut[9:5],3'b0,flashOut[4:0],3'b0});
	3'b000:		rgb <= {R(rgb_i4),3'b0,G(rgb_i4),3'b0,B(rgb_i4),3'b0};
	endcase
always @(posedge vclk)
    de <= ~blank4;
...
begin
    ...
    rstst <= `TRUE;
		ctrl[14] <= 1'b0;
		state <= OTHERS;	// abort operation, go back to idle
	end
	else begin
    pushstate <= st;
    pushst <= `TRUE;
