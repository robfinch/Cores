...
module Petajon_dba64(rst_i, hartid_i, clk_i, wc_clk_i, nmi_i, irq_i, cause_i, vpa_o, 
...
{+S100}
	XRDY_i,VI_i,NMI_ni,PWRFAIL_ni,DMA_ni,ADR_o,SDSB_ni,CDSB_ni,ADSB_ni,DODSB_ni,PHI,
	pSTVAL_no,pHLDA_o,DATA_io,sM1_o,sOUT_o,sINP_o,sMEMR_o,sHLTA_o,SLAVE_CLR_o,
	sXTRQ_no,SIXTN_no,MWRT_o,RDY_i,INT_ni,HOLD_ni,RESET_ni,pSYNC_o,pWR_no,pDBIN_o,
	sINTA_o,sWO_no,ERROR_ni,POC_ni
	// NDEF21,RFU27,RFU28,CLOCK49,NDEF65,NDEF66,PHANTOMn67,RFU69,RFU71,
{-S100}
	);
{+RV32I}
parameter WID = 32;
...
{-RV64I}
parameter FPWID = 64;
parameter ABWID = %ABWID%;
parameter RSTPC = %ResetAddress%;
...
reg [63:0] dati;
reg [63:0] datiH;
wire [63:0] datiL = wb_dat_i >> {ea[2:0],3'b0};
{+F}
...
begin
	...
	ale2 <= ale1 & lcyc;
	ale3 <= ale2 & lcyc;
	ALE_o <= (fast_access ? ale1 : ale3) & lcyc & ~bact5;
	pSYNC_o <= ALE_o & ~bact6;
end
{-S100}
...
{-WB}
{+S100}
	ADR_o <= (bact3 & ~bact4) ? ADR[31:24] : ADR[23:0];
{-S100}
...
IFETCH:
	begin
		...
		2'b01:	Dregset <= regset[24:20];
		2'b10:	Dregset <= regset[24:20] > 5'd29 ? regset[24:20]-2'd1 : ASID[4:0];
		2'b11:	Dregset <= regset[24:20];
		endcase
		case({regset[4],regset[1]})
		2'b00:	Rs1regset <= ASID[4:0];
		2'b01:	Rs1regset <= regset[24:20];
		2'b10:	Rs1regset <= regset[24:20] > 5'd29 ? regset[24:20]-2'd1 : ASID[4:0];
		2'b11:	Rs1regset <= regset[24:20];
		endcase
		case({regset[4],regset[2]})
		2'b00:	Rs2regset <= ASID[4:0];
		2'b01:	Rs2regset <= regset[24:20];
		2'b10:	Rs2regset <= regset[24:20] > 5'd29 ? regset[24:20]-2'd1 : ASID[4:0];
		2'b11:	Rs2regset <= regset[24:20];
		endcase
		case({regset[4],regset[3]})
		2'b00:	Rs3regset <= ASID[4:0];
		2'b01:	Rs3regset <= regset[24:20];
		2'b10:	Rs3regset <= regset[24:20] > 5'd29 ? regset[24:20]-2'd1 : ASID[4:0];
		2'b11:	Rs3regset <= regset[24:20];
		endcase
		state <= IFETCH2;
...
{-F}
		...
		`LOAD:
			begin
				...
				tEA();
				ARID <= 4'h1;
				if (ea[ABWID-1:ABWID-12]==12'hFFE)
					state <= AXI_MEMORY_READ;
				else
					state <= MEMORY;
			end
		`STORE:
...
{-S100}
				...
				default:	;
				endcase
				if (ea[ABWID-1:ABWID-12]==12'hFFE) begin
					AWID <= 4'h1;
					WVALID <= 1'b1;
					WSTRB <= ssel[7:0];
					WDATA <= sdat[63:0];
					state <= AXI_MEMORY_WRITE;
				end
				else
					state <= MEMORY;
			end
...
{+SBB}
		if (ol==2'b00 && ea[ABWID-1:ABWID-8]!=8'hFF) begin
			if (ea >= mbound) begin
				tAccessFault(wb_we_o ? 8'h07 : 8'h05);	// store / load access fault
			end
		end
{-SBB}
	end
...
{-F}
				...
				ARID <= 4'h2;
				wb_adr_o <= {wb_adr_o[31:3]+2'd1,3'd0};
...
task tEA;
begin
	if ((mprv ? mstatus[5:4]==2'b11 : ol > 2'b00) || %EAExclusion%)
		ladr <= ea;
	else begin
...
task tNextRs;
begin
	if (regset[21:20]!=2'b11) begin
		regset[21:20] <= regset[21:20] + 2'd1;
		regset[19:0] <= {regset[14:0],5'h1F};
	end
end
endtask
...
task tPrevRs;
begin
	if (regset[21:20]!=2'b00) begin
		regset[21:20] <= regset[21:20] - 2'd1;
		regset[19:0] <= {5'h1C,regset[19:5]};
	end
end
endtask
