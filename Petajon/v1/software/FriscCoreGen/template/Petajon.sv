...
{-RV32I}
{+RV64I}
wire [WID-1:0] datiL = wb_dat_i >> {ea[2:0],3'b0};
{-RV64I}
{+F}
...
{-WB}
{+AXI}
if (fast_access) begin
	AWADDR <= ladr;
	ARADDR <= ladr;
end
else begin
	if (ladr[ABWID-1:ABWID-8]==8'hFF) begin
		AWADDR <= ladr;
		ARADDR <= ladr;
	end
	else begin
		AWADDR <= {pagemapo,ladr[`LOG_PGSZ-1:0]};
		ARADDR <= {pagemapo,ladr[`LOG_PGSZ-1:0]};
	end
end
{-AXI}
{+S100}
...
MEMORY_WRITE:
	begin
		wb_we_o <= HIGH;
		wb_sel_o <= ssel[WID/8-1:0];
		tEA();
		wb_dat_o <= sdat[WID-1:0];
...
{-RV32I}
{+RV64I}
				wb_adr_o <= {wb_adr_o[AMSB:3]+2'd1,3'd0};
{-RV64I}
