...
module friscv4(mhartid, rst_i, clk_i, tm_clk_i, cyc_o, stb_o, ack_i, we_o, sel_o, adr_o, dat_i, dat_o, irdy_i, iadr_o, idat_i);
parameter PCMSB = 43;
input [31:0] mhartid;
input rst_i;
input clk_i;
...
begin
    ...
    RFcnt <= 1'b1;
      ...
      `ALU1,`Lx,`Sx:
        if (X32 && insn[31:25]==7'b1000000)
          ...
          pc <= pc + 32'd4;
      7'b1111111: pc <= pc + 32'd16;  // detect 128 bit op
      default:  pc <= pc + 32'd4;
      endcase
      dcs <= cs;
		end
	...
	if (advanceRF) begin
	  EXcnt <= RFcnt;
		...
		`ALU1:	imm <= di32 ? ir[63:32] : xisLuiz ? {xir[31:12],ir[31:20]} : {{20{ir[31]}},ir[31:20]};
		default:	imm <= 32'd0;
		endcase
		case(opcode)
		`LUI:	xRt <= ir[11:7];
