...
static void emit_postfix8(Int128 val)
{
	if (!IsNBit128(val, Int128(8))) {
//	if (val < -127 || val > 127) {
		emit_insn(((val.low >> 12LL)<<8LL)|0xC0|((val.low >> 8LL)&0xF),0,5);
	}
	if (!IsNBit128(val,Int128(44))) {
//	if (val < -0x7FFFFFFFFFFLL || val > 0x7FFFFFFFFFF) {
		Int128 tmp44;
		Int128 tmp48;

		tmp44.Shr(&tmp44, &val, 44LL);
		tmp48.Shr(&tmp48, &val, 48LL);
		//	if (val < -0x7FFFFFFFFFFFFFLL || val > 0x7FFFFFFFFFFFFFLL) {
		emit_insn(((tmp48.low) << 8LL) | 0xC0 | ((tmp44.low) & 0xF), 0, 5);
...
static void emit_postfix20(Int128 val)
{
	if (!IsNBit128(val, Int128(20))) {
//	if (val < IMIN20 || val > IMAX20) {
		emit_insn((((unsigned __int64)val.low >> 24LL)<<8LL)|0xC0|((val.low >> 20LL)&0xF),0,5);
	}
	if (!IsNBit128(val,Int128(56))) {
		Int128 tmp56;
		Int128 tmp60;

		tmp56.Shr(&tmp56, &val, 56LL);
		tmp60.Shr(&tmp60, &val, 60LL);
//	if (val < -0x7FFFFFFFFFFFFFLL || val > 0x7FFFFFFFFFFFFFLL) {
		emit_insn(((tmp60.low)<<8LL)|0xC0|((tmp56.low)&0xF),0,5);
	}
}
...
static void process_riop(int opcode6)
{
	...
	*/
	emit_insn(((val.low & 0xFFFFFLL) << 20)|(Rt << 14)|(Ra << 8)|opcode6,0,5);
	emit_postfix20(val);
}
...
//        case tk_fstat: process_fprdstat(0x86); break;
        ...
        case tk_ldwu: process_load(0x83); break;
				case tk_ldo: process_load(0x8A); break;
				case tk_ldou: process_load(0x8B); break;
...
//        case tk_not: process_rop(0x07); break;
        ...
        case tk_stt: process_store(0x94); break;
				case tk_sto: process_store(0x97); break;
