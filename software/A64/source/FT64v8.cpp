...
static void process_rrop()
{
  ...
  Rb = getRegisterX();
	...
	if (funct6==0x3C || funct6==0x3D || funct6==0x3E) {
	    emit_insn((funct6<<26LL)||(0<<23)|RB(Rb)|RT(Rt)|RA(Ra)|0x02,!expand_flag,4);
			goto xit;
	}
    emit_insn(instr | (funct6<<26LL)|(sz << 23)|RB(Rb)|RT(Rt)|RA(Ra)|0x02,!expand_flag,4);
	xit:
		prevToken();
		ScanToEOL();
}
...
static void process_shift(int64_t op4)
{
	...
	if (token=='#') {
		...
		emit_insn((func6 << 26) | (op4 << 23) | RT(Rt)| RB(Rb) | RA(Ra) | 0x02,!expand_flag,4);
	 }
}
