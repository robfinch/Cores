...
void Function::SetupReturnBlock()
{
	...
	cg.GenStore(makereg(regFP), MakeIndirect(regSP), sizeOfWord);
	cg.GenStore(makereg(regZero), MakeIndexed(sizeOfWord, regSP), sizeOfWord);
...
void Function::GenReturn(Statement *stmt)
{
	...
	if (stmt != NULL && stmt->exp != NULL)
	{
		...
		if (isFloat)
			ap = stmt->exp->Generate(am_fpreg, sizeOfFP);
		else
			ap = stmt->exp->Generate(am_reg | am_imm, sizeOfWord);
		...
		if (ap->mode == am_imm)
			...
			if (sym->tp->GetBtp() && (sym->tp->GetBtp()->type == bt_struct || sym->tp->GetBtp()->type == bt_union || sym->tp->GetBtp()->type == bt_class)) {
				...
				if (p) {
					if (p->IsRegister)
						GenerateDiadic(op_mov, 0, makereg(regV0), makereg(p->reg));
					else
						GenerateDiadic(op_ldo, 0, makereg(regV0), MakeIndexed(p->value.i, regFP));
					ap2 = GetTempRegister();
					GenerateDiadic(op_ldi, 0, ap2, MakeImmediate(sym->tp->GetBtp()->size));
					if (cpu.SupportsPush) {
						GenerateMonadic(op_push, 0, ap2);
						GenerateMonadic(op_push, 0, ap);
						GenerateMonadic(op_push, 0, makereg(regV0));
					}
					else {
						GenerateTriadic(op_sub, 0, makereg(regSP), makereg(regSP), MakeImmediate(sizeOfWord * 3));
						GenerateDiadic(op_sto, 0, makereg(regV0), MakeIndirect(regSP));
				...
				if (sym->tp->GetBtp()->IsFloatType())
					GenerateDiadic(op_fmov, 0, makefpreg(regV0), ap);
				else if (sym->tp->GetBtp()->IsVectorType())
					GenerateDiadic(op_mov, 0, makevreg(regV0), ap);
				else
			...
			if (isFloat)
				GenerateDiadic(op_lf, 'd', makereg(regV0), ap);
			else {
				cg.GenLoad(makereg(regV0), ap, sizeOfWord, sizeOfWord);
				//GenerateDiadic(op_ldd, 0, makereg(1), ap);
			}
		}
		else {
			if (sym->tp->GetBtp()->IsVectorType())
				GenLoad(makevreg(regV0), ap, sizeOfWord, sizeOfWord);
			else
				GenLoad(makereg(regV0), ap, sizeOfWord, sizeOfWord);
		}
		ReleaseTempRegister(ap);
	}
...
void Function::BuildParameterList(int *num, int *numa)
{
	...
	*numa = 0;
	...
	for (i = 0; i < np && i < MAX_PARMS; ++i) {
		...
		if (round10(sp1->tp->size) > 10 && !sp1->tp->IsVectorType())
			IsLeaf = FALSE;
