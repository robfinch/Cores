...
char errMsgBuf[128];
	...
	Err::Err(int n)
	{
		...
		nn = sprintf(errMsgBuf, ERR_MSG_PREFIX " %c%03d (line %d): %s\r\n", errch, num, theAssembler.getLineno(), &error[num-2001][1]);

		if (errch == 'E')
			theAssembler.incErrorsDuringPass();
		// Only output err on second pass,
		if (theAssembler.isGenerationPass() || theAssembler.isForcedErr()) {
			fputs(errMsgBuf, theAssembler.getErrFp());
			if (errch == 'E')
				theAssembler.incErrors();
			else
	...
	Err::Err(int n , const char *s)
	{
		...
		tmp = errno;
		errch = error[num-2001][0];
		nn = sprintf(errMsgBuf, ERR_MSG_PREFIX " %c%03d (line %d): ", errch, num, theAssembler.getLineno());
		nn += sprintf(&errMsgBuf[nn], &error[num-2001][1], s);
	...
	Err::Err(int n, int m)
	{
		...
		errch = error[num-2001][0];
		nn = sprintf(errMsgBuf, ERR_MSG_PREFIX " %c%03d (line %d): ", errch, num, theAssembler.getLineno());
		nn += sprintf(&errMsgBuf[nn], &error[num-2001][1], m);
		sprintf(&errMsgBuf[nn], "\r\n");
